(function (global, factory) {
    ...
    function InitWpsCloudSvr() {
        if (serverId == undefined) {
            if (!document.getElementById('__kd_wps__')) {
                var iframeElement = document.createElement('iframe')
                iframeElement.src = "ksoWPSCloudSvr://start=RelayHttpServer"
                iframeElement.id = '__kd_wps__'
                document.body.appendChild(iframeElement)
            } else {
                document.getElementById('__kd_wps__').src = document.getElementById('__kd_wps__').src
            }
        }
    ...
    function RegWebNotify(clientType, name, callback, wpsclient) {
        ...
        // 苍穹表单暂时无需与客户端有直接沟通，所以先注释掉
        // 不然在网页会一直发送 askNotify 请求
        // askItem()
    }
    ...
    function WpsAddonGetAllConfig(callBack) {
        ...
        startWps({
            ...
            sendData: baseData,
            callback: callBack,
            tryCount: WpsAddonMgr.tryCount || 3,
            bPop: true,
            timeout: 5000,
            concurrent: true
        });
