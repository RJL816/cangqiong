	...
	if (G.currentNode) {// 删除节点
		...
		for (var i = 0; i < G.currentNodes.length; i++) {
			...
			if (!G.couldBeDeleted(node)) {
				var tips = res("life.sys.tips.could_not_be_deleted", $(node)
						.text());
		...
		M.showConfirm(res("life.sys.warn.delete_node"),
				res("life.sys.label.delete"), function() {
					for (var i = 0; i < G.currentNodes.length; i++) {
		...
		M.showConfirm(res("life.sys.warn.delete_link"),
				res("life.sys.label.delete"), function() {
					G.hidePorts();
...
G.decorateNode = function(node, category) {
	if (!G.editable) {
		...
		$(node).mouseover(function() {// 光标移到节点上
			$(this).addClass("hover");
			$(this.lastElementChild).css("display","flex");
		}).mouseout(function() {// 光标移出节点
			$(this).removeClass("hover");
...
// 添加一个节点
G.addNode = function(html, x, y, type, width, height,tips) {
	...
	var nodeRunningState = "nodeRunningState_" + tips.state;// 配置节点div的class
	var nodeRunningTitle =  G.getTitle(tips) ;// 配置节点div的title
	html2 = "<div class='"+ nodeRunningState + "'" + "title='"+ nodeRunningTitle+"'>" + html2 + "</div>";
	
	
	if(JSON.parse(tips.state).direct == "up"){
		var icon = "left_arrow.svg";
		var arrow = "<img src='img/" + icon + "' width='24' height='24' onClick = 'G.viewSource(this,event)' title = '单击此处查看以此节点为中心的资源图。'>";
		html2 += "<div class='left_arrow'>" + arrow + "</div>";
	}else if(JSON.parse(tips.state).direct == "down"){
		var icon = "right_arrow.svg";
		var arrow = "<img src='img/" + icon + "' width='24' height='24' onClick = 'G.viewSource(this,event)' title = '单击此处查看以此节点为中心的资源图。'>";
		html2 += "<div class='right_arrow'>" + arrow + "</div>";
	}else{
		html2 += "<div class='empty_div'></div>";
	}
...
G.showMenu = function(node) {
	...
	if (!menu) {
		var html = "<div id='__node_menu' class='node_menu'>"
				...
				+ res("life.sys.tips.add_node") + "'></span>"
				+ "<br><span class='remove_node'"
				+ " onclick='G.menuDeleteNode()' title='"
				+ res("life.sys.tips.delete_node") + "'></span></div>";
