 ...
 *  自定义控件书写模板
 */
  var vehicleExpenseChart;
(function (KDApi, $) {
    // 构造函数，变量名随意，与最后一句代码的KDApi.register的第二个参数一致即可
    function VehicleExpense(model) {
        this._setModel(model)
    }

    // 原型中封装生命周期函数，固定格式
    var isUpdate = false;
    VehicleExpense.prototype = {
    ...
    var setHtml = function (model, props, isUpdate) {
		if(!isUpdate){
			   KDApi.templateFilePath('./html/container_vehicleexpense.html', model, {}).then(
                function (result) {
                    model.dom.innerHTML = result;
                    var data=props.data;
		if(!data){
			var chartDom = document.getElementById('container_trip_vehicleexpense');
			var emptyDom=document.getElementsByClassName('kd-cq-empty-vehicleexpense');
        ...
        if (isUpdate&&props.data&&Object.keys(props.data).length>0) {
		 KDApi.loadFile("./js/echarts.min.vehicleexpense.js", model, function (){
            if(echarts){
         		   vehicleExpenseChart=echarts;
         		   }
			 KDApi.loadFile("./js/echarts-liquidfill.js", model, function () {
                ...
                function (result) {
                    model.dom.innerHTML = result;
                    loadData(model, props,vehicleExpenseChart);
                }
            )
    ...
    var loadData = function (model, props,vehicleExpenseChart) {
        var option = {};
        var chartDom = document.getElementById('container_trip_vehicleexpense');
        var myChart = vehicleExpenseChart.init(chartDom);
		...
		if(data&&data.length==0||JSON.stringify(data)=='{}'){
			...
			chartDom.setAttribute("style","display:block;height:200px;width:100%");
	  ...
	  // 使用 graphic 绘制光晕效果
            graphic: {
                elements: [{
                 ...
                 style: {
                        fill: 'none',
                        stroke: new vehicleExpenseChart.graphic.LinearGradient(
                            0, 0, 1, 1,  // 渐变方向
                            [
    ...
    KDApi.register('kingdee-fi-vehicleexpense', VehicleExpense)
