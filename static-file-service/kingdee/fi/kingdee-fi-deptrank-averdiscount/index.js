...
(function (KDApi, $) {
    ...
    function DeptRankAverageDiscount(model) {
        this._setModel(model)
    }

    // 原型中封装生命周期函数，固定格式
    var isUpdate = false;
    DeptRankAverageDiscount.prototype = {
    ...
    var setHtml = function (model, props, isUpdate) {
		...
		KDApi.loadFile(jsPath, model, function () {
        			 if (isUpdate&&props.data&&props.data.averageDiscount.others.length>0) {
        				   KDApi.templateFilePath('./html/container_deptrank.html', model, {}).then(
                        function (result) {
                        ...
                        function (result) {
                            model.dom.innerHTML = result;
                            var chartDom = document.getElementById('dept-rank-averagediscount-table-container');
						 var emptyDom = document.getElementsByClassName('kd-cq-empty-averagediscount');
        ...
        function createRankingContainer(top3) {
            const content = document.getElementById('dept-rank-averagediscount-content');
			var w=content.style.width;
			var h=content.style.height;
            const container = document.getElementById('dept-rank-averagediscount-ranking-container');
            ...
            top3.forEach(item => {
                ...
                rankingItem.innerHTML = `
                    ...
                    <div class="orders">${item.discount}</div>
                `;
        ...
        function createTable(others) {
          const content = document.getElementById('dept-rank-averagediscount-table-container');
		    const emptyDom = document.getElementsByClassName('kd-cq-empty-averagediscount');
			   	emptyDom[0].setAttribute("style","display:none");
		content.setAttribute("style","display:block;");
            const table = document.createElement('table');
			table.setAttribute("id","dept-rank-averagediscount-innertable");
    ...
    var loadData = function (model, props) {
           ...
           // const rankingContainer = createRankingContainer(props.data.averageDiscount.top3);
            const table = createTable(props.data.averageDiscount.others);
    ...
    KDApi.register('kingdee-fi-deptrank-averdiscount', DeptRankAverageDiscount)
