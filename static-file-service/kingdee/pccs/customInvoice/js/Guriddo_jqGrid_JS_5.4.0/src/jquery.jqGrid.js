...
$.extend($.jgrid,{
	...
	hasSpan: function(ts) {
        return !!ts.p.rowSpanCols || !!ts.p.colspans;
    },
    ...
    move: function(number, step) {
        number = Number(number) || 0;//如果传过来的是""返回NaN，会导致调用的函数报错
        var negative = number < 0,
            numberStr = new String(number),
            pointIdx = numberStr.indexOf("."),
            exponentialIdx = numberStr.toLowerCase().indexOf("e"),
            exponentialStr = "",
            left,
            right,
            result;
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
            isClouds: false, // 用于判断是否初始化表格，由于苍穹平台发票页面是同一个页面，每次打开重新加载自定义控件，需要重新初始化
			url: "",
			height: 150,
			page: 1,
			rowNum: 20,
		...
		getOffset = function (iCol) {
			...
			var i, ret = {},
                brd1 = $.jgrid.cellWidth(ts) ? 0 : ts.p.cellLayout;
            ret[0] = ret[1] = ret[2] = 0;
            for (i = 0; i <= iCol; i++) {
                if (ts.p.colModel[i].hidden === false) {
                    ret[0] += parseFloat(ts.p.colModel[i].width) + brd1;
                }
            }
            if (ts.p.direction == "rtl") {
                ret[0] = ts.p.width - ret[0];
            }
            if (!ts.p.colModel[iCol].frozen) {
            ...
            if ($(ts.grid.hSelectAllDiv).length > 0) {
                ret[1] += $(ts.grid.hSelectAllDiv).outerHeight();
            }

			ret[2] += $(ts.grid.bDiv).height() + $(ts.grid.hDiv).height();
            if (ts.grid.sDiv && $(ts.grid.sDiv).is(":visible")) {
                ret[2] += $(ts.grid.sDiv).height();
            }
			return ret;
		},
	...
	getCellObject: function(rowid, col) {
        ...
        this.each(function() {
            ...
            if (pos >= 0) {
                ...
                //     if (ret) ret = $(ret);
                // } else {
                //     ret = $(ind).find("td[role=gridcell]:eq(" + pos + ")");
                // }
                ret = $(ind).find("td[role=gridcell]:eq(" + pos + ")");
            }
        });
	...
	editCell : function (iRow,iCol, ed, event){
		return this.each(function (){
			...
			if (cm.editable===true && ed===true && !cc.hasClass("not-editable-cell") && (!$.isFunction($t.p.isCellEditable) || $t.p.isCellEditable.call($t,nm,iRow,iCol))) {
				...
				if( $.inArray(cm.edittype, ['text','textarea','password','select', 'f7']) > -1) {
					// $(elc).addClass(inpclass);
                    $('#' + opt.id).addClass(inpclass);
				}

				cc.html("").append(elc);
                $('#' + opt.id).attr("tabindex","0");
...
// Form Functions
	createEl : function(eltype,options,vl,autowidth, ajaxso) {
		...
		switch (eltype)
		{
			...
			break;
            case "f7" :
                var opts = $($t).jqGrid('getColProp', options.name);
                // console.log(vl);
                if(vl || parseInt(vl) == 0){
                	if(opts.editoptions.f7Json.tips == 'per' && vl.indexOf('%') < 0 && (typeof vl === "number")) {
                        vl = parseInt(vl)+ "%";
					}
                }else {
                    vl = "";
                ...
                var elemstr = '';
                elemstr += '<div class="ui-f7-frame">';
                elemstr += '	<div class="ui-f7-layout">';
                elemstr += '		<div class="ui-f7-inputframe">';
                elemstr += '			<input id="'+ options.id +'" type="text" role="f7" readonly value="'+ vl +'" class="enter2tab waf2_field ui-f7" name="'+ options.name +'" rowid="'+ options.rowId +'">';
                elemstr += '		</div>';
                elemstr += '		<div class="ui-f7-icon customInvoice-f7-icon">';
                elemstr += '			<i class="iconfont icon-fangdajing"></i>';
                elemstr += '		</div>';
                elemstr += '	</div>';
                elemstr += '</div>';

                var cellobj = $($t).jqGrid('getCellObject', options.rowId, options.name);
                cellobj.on('click', '.customInvoice-f7-icon', function(){
					
					
                    // iconTriggerClick
					// console.log(options);
					var rowid = options.rowId;
					// jqGird的表格的行id是从1开始，此处需要-1
					var realrowid = parseInt(rowid) - 1;
					var curcolName = options.name;
					var elemid = options.id;

					// 获取隐藏表格
					var triggerTableId = options.f7Json.triggerTablePanelId;
                    var $entry = $('[data-page-id="' + triggerTableId + '"]');
					// var cptable =$entry.find('.ag-body-container');
                    var cptable =$entry.find('tbody');
				
                    if(cptable.length > 0) {
                        var allrows = cptable.find('tr[role="row"]');
                        var currentRow = cptable.find('tr[data-row-key="'+ realrowid +'"]');
                  
                        var curcell = $('td[data-code="'+ curcolName +'"]', currentRow);
						var curcell2 = currentRow.find('td[data-code="'+ curcolName +'"]');
                        console.log('点击的当前单元格是');
                         console.log(curcell);
                        curcell.click();
                        var curicon = curcell.find('span.kdfont-f');
                        ...
                        // curicon.click();
                        curicon[0].dispatchEvent(mouseEvent);
						cellobj.off("click",".customInvoice-f7-icon");
						
					}else {
                    	console.log("找不到发票自定义隐藏苍穹分录控件");
                    	return false;
					}
			...
			default :
				...
				// 设置最大输入长度
                var opts = $($t).jqGrid('getColProp', options.name);
                if(opts.editrules && opts.editrules.maxlength){
                    $(elem).attr('maxlength', opts.editrules.maxlength);
				}

				// formatter为number时只能输入数字
				if(opts.formatter === "number"){
                    $(elem).blur(function(){
                    	var currentVal = this.value;
                        var allowDecimals = true;
                        opts.roundType = "round";
                        if (currentVal == null || currentVal == "") return currentVal;
                        if (allowDecimals && opts.formatoptions.decimalPlaces != 0) {
                            var precision = opts.formatoptions.decimalPlaces,
                                tmpValue = $.jgrid.move(currentVal, precision);
                            //先做取舍，然后移动精度
                            currentVal =  $.jgrid.move(Math[opts.roundType](tmpValue), 0 - precision);
                        } else {
                            currentVal =  Math[opts.roundType](currentVal);
                        }

                        $(elem).val($.fmatter.util.NumberFormat(currentVal,opts));
					});
				}
...
//module begin
	...
	$.fmatter.util = {
		...
		NumberFormat : function(nData,opts) {
			...
			if($.fmatter.isNumber(nData)) {
				...
				var nDotIndex;
                opts.decimalPlaces = opts.decimalPlaces || opts.formatoptions.decimalPlaces;
				if($.fmatter.isNumber(opts.decimalPlaces)) {
					// Round to the correct decimal place
					var nDecimalPlaces = opts.decimalPlaces;
					var nDecimal = Math.pow(10, nDecimalPlaces);
                    nData = Math.round(nData * nDecimal) / nDecimal + "";

                    //更改处理精度问题，原有方式不支持精度是负数的情况
                    sOutput = $.jgrid.formatNumber(nData, opts.decimalPlaces);
    ...
    $.fn.fmatter.f7 = function (cellval,opts) {
    	if(cellval == null) {
            cellval = "";
		}
        if(typeof cellval === "string" && cellval === ""){
            return "";
        }else if(opts.colModel.editoptions.f7Json.tips == 'per' && (typeof cellval === "number")){
        	cellval = cellval + '';
	...
	$.unformat = function (cellval,options,pos,cnt) {
		...
		if(unformatFunc !== undefined && $.isFunction(unformatFunc) ) {
			...
			switch(formatType) {
                ...
                case 'f7' :
                	// 自定义格式化f7显示
                    ret = $.unformat.f7(cellval,options,pos,cnt);
                    break;
				case 'actions':
					return "";
				default:
    ...
    $.unformat.f7 = function (cellval,options,pos,cnt) {
        return $(cellval).text();
	};
