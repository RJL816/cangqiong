...
function Datepicker() {
    ...
    this._frameClass = 'ui-datepicker-frame';
	...
	this._defaults = { // Global defaults for all the date picker instances
        ...
        formBillId: '', // 苍穹平台单据布局问题，需要添加一个属性，用来重新设置日期控件dom元素的位置，为了实现滚动时日期控件跟着滚动
        calcPosObj: {}, // 苍穹平台，用来计算日期控件的position
        invoicePageId: '' // 苍穹平台，当前页面pageid，用来做初始化日期插件的判断
	};
...
$.extend(Datepicker.prototype, {
	...
	_showDatepicker: function(input) {
        ...
        // 获取苍穹单据控件，调整日期控件的位置，为了实现往下滚动，日期控件跟着滚动
        // var formBillId = $.datepicker._get(inst, "formBillId");
        var invoicePageId = $.datepicker._get(inst, "invoicePageId");
        var calcPosObj = $.datepicker._get(inst, "calcPosObj");
        var $titlepanel = $('[data-page-id="' + calcPosObj.titlepanelflexPageId + '"]');
        var $contentpanel = $('[data-page-id="' + calcPosObj.contentpanelPageId + '"]');
        var $fs_baseinfo = $('[data-page-id="' + calcPosObj.fs_baseinfoPageId + '"]');
        if(invoicePageId && invoicePageId!== ''){
            // 苍穹平台，日期控件dom位置调整，需要重新设置日期控件显示的位置
            // 相对于data-form-id="concs_invoicebill" 的绝对定位
            inst.dpDiv.css({top:0, right:0, position: 'absolute', display: 'block'});
            $.datepicker._updateDatepicker(inst);

            // 苍穹平台特殊处理，这里由于日期控件初始化dom位置做了特殊调整，所以这里定位写了绝对值而不是动态计算
			var datepickerTop = 0;
            datepickerTop += $titlepanel.height() + 10;
            datepickerTop += $('[data-page-id="' + calcPosObj.contentpanelPageId + '"]>div').height() + 8;
            datepickerTop += 60;

            // inst.dpDiv.css({
            //     top: datepickerTop || 0,
            //     left: inst.input.offset().left - 267
            // });
            inst.dpDiv.css({
                top: datepickerTop || 0,
                right: 42
				...
				for (dRow = 0; dRow < numRows; dRow++) { // create date picker rows
					...
					for (dow = 0; dow < 7; dow++) { // create date picker days
						...
						tbody += "<td class='" +
							...
							(printDate.getTime() === today.getTime() ? " ui-state-highlight iconfont" : "") +
...
$.fn.datepicker = function(options){
    ...
    var invoicePageId = options.invoicePageId;

    if(invoicePageId) {
        $.datepicker._mainDivId = invoicePageId + '_' + $.datepicker._mainDivId;
		var len = $('.ui-datepicker').length;

        if(len < 1) {
            $.datepicker.dpDiv.attr('id', $.datepicker._mainDivId);
		}else {
	...
	if ($("#" + $.datepicker._mainDivId).length === 0) {
        ...
        var baseTarget;
        if(invoicePageId && invoicePageId!== ''){
            baseTarget = $('div[data-page-id="'+ invoicePageId +'"]');
		}

        if(baseTarget) {
            $.datepicker.dpDiv.appendTo(baseTarget);
        }else {
