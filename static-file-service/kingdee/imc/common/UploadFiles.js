    ...
    try {
      ...
      $.ajax({
        ...
        error: function error(res) {
		  ...
		  let responseText = res.responseJSON;
			if (responseText) {
				if (responseText.url && responseText.status === 'success') {
					console.warn('upload fail3', responseText.url,responseText.status);
...
function _upload() {
  _upload = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(url, files) {
    var opt,
        maxFileSize,
        fileQuality,
        fileLimitWidth,
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            ...
            maxFileSize = opt.maxFileSize, fileQuality = opt.fileQuality, fileLimitWidth = opt.fileLimitWidth, fileLimitHeight = opt.fileLimitHeight, fileLimitPixel = opt.fileLimitPixel, fileLimitSize = opt.fileLimitSize, _opt$uploadData = opt.uploadData, uploadData = _opt$uploadData === void 0 ? {} : _opt$uploadData, _opt$fieldName = opt.fieldName, fieldName = _opt$fieldName === void 0 ? 'file' : _opt$fieldName, _opt$stepUploadStart = opt.stepUploadStart, stepUploadStart = _opt$stepUploadStart === void 0 ? function (f) {
              return f;
            } : _opt$stepUploadStart, _opt$stepUploadFinish = opt.stepUploadFinish, stepUploadFinish = _opt$stepUploadFinish === void 0 ? function (f) {
              return f;
          ...
          case 36:
            ...
            mSize = _curFile.size / 1024 / 1024;

            if (!(maxFileSize && mSize > maxFileSize)) {
              _context.next = 45;
              break;
            }
            ...
            return stepUploadFinish({
              errcode: '30002',
              description: '文件大小超出限制(' + maxFileSize + 'M)，请检查!',
              file: _curFile,
              localUrl: '',
              uid: _curUid
            });
          ...
          case 47:
            ...
            _context.next = 51;
            return stepUploadFinish(_objectSpread(_objectSpread({}, res), {}, {
              file: _curFile,
          ...
          case 58:
            ...
            if (preRes && preRes.errcode === '0000') {
              preResData = preRes.data || {};
            }

            fullData = _objectSpread(_objectSpread({}, uploadData), preResData);
            keyNames = Object.keys(fullData);
            formData = new FormData();
            formData.append(fieldName, file);

            for (j = 0; j < keyNames.length; j++) {
              formData.append(keyNames[j], fullData[keyNames[j]]);
            }
          ...
          case 67:
            ...
            _context.next = 70;
            return stepUploadFinish(_objectSpread(_objectSpread({}, resUpload), {}, {
              file: _curFile,
              localUrl: localUrl,
              uid: _curUid
            }));
