var FPY_workbenchFun = function (KDApi, $, _) {
    const INVOICE_TYPES = [
    ...
    function showBillParam() {
        ...
        let iselepaper = billList[0]["iselepaper"];

        // 数电不展示设备编号
        if (!(invoicetype === '08xdp' || invoicetype === '10xdp' || iselepaper === '1')) {
            showBillDeviceNo()
        } else {
    ...
    function updateBillHeader() {
        ...
        const itemCountSum = billList.length ? billList.reduce((all, b) => fpy_Add(all, b.itemcount), []) : 0;
        ...
        $bill_footer_span.eq(2).text(itemCountSum + '个');
    }
                ...
                <div class="item">
                    ...
                    ${isExistGoodsTransport ? (
                        `<div>
                            ...
                            <div id="imc_invoiceWorkbench_treatment_goodsTransport${obj.id}"></div>
                        </div>`
                    ) : ''}
                    ${isExistGoodsTravel ? (
                        `<div style="overflow-x: scroll;overflow-y: hidden;">
                            <div id="imc_invoiceWorkbench_treatment_goodsTravel${obj.id}"></div>
                        </div>`
                    ) : ''}
                    ${isExistGoodsCollectionShip ? (
                        `<div style="overflow-x: scroll;overflow-y: hidden;">
                            <div id="imc_invoiceWorkbench_treatment_goodsCollectionShip${obj.id}"></div>
                        </div>`
                    ) : ''}
                </div>
            `);
    ...
    function updateTreatmentInvoiceInfoHtmlForBuyernameTable(invoiceIndex, setScrollTop = false) {
        ...
        let tableDom;
        if (setScrollTop) {
            tableDom = $(`#imc_invoiceWorkbench_treatment_invoice_buyername${id}`);
            scrollTop = tableDom.find('.workbench-scroll').scrollTop();
        }
        fpyTable.init({
            id: `imc_invoiceWorkbench_treatment_invoice_buyername${id}`,
            rowKey,
            columns,
    ...
    function updateTreatmentInvoiceInfoHtmlForeStatesalesTable(invoiceIndex, setScrollTop = false) {
        ...
        let tableDom;
        if (setScrollTop) {
            tableDom = $(`#imc_invoiceWorkbench_treatment_invoice_statesales${id}`);
            scrollTop = tableDom.find('.workbench-scroll').scrollTop();
        }
        fpyTable.init({
            id: `imc_invoiceWorkbench_treatment_invoice_statesales${id}`,
            rowKey,
            columns,
    ...
    function getTreatmentInvoiceInfoHtmlByIndex(invoiceIndex) {
        const { id: billId, splitrule, iselepaper } = treatmentShowBillList[treatmentIndex];
        // 是否按明细拆分
        const isDetailSplit = splitrule === IMC_SPLIT_BY_DETAIL;
        const { showFixedNumber, fixedNumber, taxFreeAdjust, hadHzxxbm } = treatmentInvoiceListSetMap[billId][invoiceIndex];
        const _data = treatmentInvoiceList[billId];
        const _length = _data.length;
        const {
            invoicetype,
        ...
        `];
        if (totalamount < 0) {
            if ((invoicetype === '028' || invoicetype === '004') && iselepaper != '1') {
                _html.push(`
                    <div class="text">
                        <div class="name requiredFields">申请方类型</div>
                        <div class="value">
                            <form>
                                <input id="applicant2${invoiceIndex}" class="searchOther vaMiddle" type="radio" name="applicant" value="2" ${applicant === '2' ? 'checked' : ''}>
                                <label for="applicant2${invoiceIndex}" class="mgl5 mgr10 vaMiddle">销方申请</label>
                                <input id="applicant1${invoiceIndex}" class="searchOther vaMiddle" type="radio" name="applicant" value="1" ${applicant !== '2' ? 'checked' : ''}>
                                <label for="applicant1${invoiceIndex}" class="mgl5 mgr10 vaMiddle">购方申请</label>
                            </form>
                        </div>
                    </div>
                    <div class="text">
                        <div class="name">红字信息编码</div>
                        <div class="value search">
                            <input class="searchInput pressEnter" type="text" name="infocode" maxlength="24" value="${infocode}" ${hadHzxxbm ? 'disabled' : ''}>
                            <div data-name="infocode" class="search-btn ${hadHzxxbm ? 'disabled' : ''}"></div>
                        </div>
                    </div>
                    <div class="text">
                        <div class="name">原蓝票代码</div>
                        <div class="value search">
                            <input class="searchInput pressEnter" type="text" name="originalinvoicecode" maxlength="12" value="${originalinvoicecode}" ${applicant !== '2' || hadHzxxbm ? 'disabled' : ''}>
                            <div data-name="originalinvoicecode" class="search-btn ${applicant !== '2' || hadHzxxbm ? 'disabled' : ''}"></div>
                        </div>
                    </div>
                    <div class="text">
                        <div class="name">原蓝票号码</div>
                        <div class="value">
                            <input class="searchInput" type="text" name="originalinvoiceno" maxlength="8" value="${originalinvoiceno}" ${applicant !== '2' || hadHzxxbm ? 'disabled' : ''}>
                        </div>
                    </div>
                    <div class="text">
                        ...
                        <div class="value">
                            <form>
                                <input id="invoiceContent1${invoiceIndex}" class="searchOther vaMiddle" type="radio" name="invoiceContent" value="1" ${invoiceContent !== '2' ? 'checked' : ''} ${_length > 1 ? 'disabled' : ''}>
                                <label for="invoiceContent1${invoiceIndex}" class="mgl5 mgr10 vaMiddle">按红字信息表开票</label>
                                <input id="invoiceContent2${invoiceIndex}" class="searchOther vaMiddle" type="radio" name="invoiceContent" value="2" ${invoiceContent === '2' ? 'checked' : ''} ${_length > 1 ? 'disabled' : ''}>
                                <label for="invoiceContent2${invoiceIndex}" class="mgl5 mgr10 vaMiddle">按负数单据开票</label>
                            </form>
                        </div>
                    </div>
                    <div class="text">
                        <div class="name">原蓝票开票日期</div>
                        <div class="value">
                            <input class="searchInput" type="date" name="originalissuetime" value="${(originalissuetime || '').substring(0, 10)}" ${applicant !== '2' || hadHzxxbm ? 'disabled' : ''}>
                        </div>
                    </div>
                    <div class="text">
                        <div class="name">备注</div>
                        <div class="value">
                            <textarea name="applyreason" maxlength="100">${applyreason}</textarea>
                        </div>
                    </div>
                    ${invoiceContent === '2'
                        ? (
                            `<div class="text set">
                                <div class="set-item">
                                    <label for="taxFreeAdjust${invoiceIndex}" class="set-name">税额调整</label>
                                    <div class="switch">
                                        <input id="taxFreeAdjust${invoiceIndex}" type="checkbox" name="taxFreeAdjust" ${taxFreeAdjust ? 'checked' : ''}>
                                        <div class="label"></div>
                                    </div>
                                </div>
                            </div>`
                        ...
                        : ''}
                `);
            } else if (invoicetype === '08xdp' || invoicetype === '10xdp' || iselepaper == '1') {
                _html.push(`
                    <div class="text">
                        <div class="name requiredFields">红字确认单</div>
                        <div class="value search">
                            <input class="searchInput pressEnter" type="text" name="infocode" maxlength="20" value="${infocode}" disabled>
                            <div data-name="infocode" class="search-btn"></div>
                        </div>
                    </div>
                    <div class="text">
                        <div class="name requiredFields">原蓝票号码</div>
                        <div class="value">
                            <input class="searchInput" type="text" name="originalinvoiceno" maxlength="8" value="${originalinvoiceno}" disabled>
                        </div>
                    </div>
					<div class="text">
                        <div class="name requiredFields">冲红原因</div>
                        <div class="value">
                            <select class="searchInput" name="redreason" disabled>
                    ...
                    <div class="text">
                        <div class="name">备注</div>
                        <div class="value">
                            <textarea name="remark" maxlength="100">${remark || ''}</textarea>
                        </div>
                    </div>
                `);
            } else {
                _html.push(`
                    <div class="text">
                        ...
                        <div class="value search">
                            <input class="searchInput pressEnter" type="text" name="originalinvoicecode" maxlength="12" value="${originalinvoicecode}">
                            <div data-name="originalinvoicecode" class="search-btn"></div>
                        </div>
                    </div>
                    <div class="text">
                        <div class="name requiredFields">原蓝票号码</div>
                        <div class="value">
                            <input class="searchInput" type="text" name="originalinvoiceno" maxlength="8" value="${originalinvoiceno}">
                        </div>
                    </div>
					<div class="text">
                        <div class="name requiredFields">原发票类型</div>
                        <div class="value">
                            <select class="searchInput" name="originalinvoicetype" value="${originalinvoicetype}">
                                <option value="" ${!originalinvoicetype ? 'selected' : ''} disabled>请选择</option>
                                <option value="026" ${originalinvoicetype === '026' ? 'selected' : ''}>电子普通发票</option>
                                <option value="007" ${originalinvoicetype === '007' ? 'selected' : ''}>纸质普通发票</option>
                                <option value="025" ${originalinvoicetype === '025' ? 'selected' : ''}>普通纸质卷票</option>
                            </select>
                        </div>
					</div>
                    <div class="text">
                        <div class="name requiredFields">原蓝票开票日期</div>
                        <div class="value">
                            <input class="searchInput" type="date" name="originalissuetime" value="${(originalissuetime || '').substring(0, 10)}">
                        </div>
                    </div>
					<div class="text">
                        <div class="name requiredFields">冲红原因</div>
                    ...
                    <div class="text">
                        <div class="name">备注</div>
                        <div class="value">
                            <textarea name="remark" maxlength="230">${remark}</textarea>
                        </div>
                    </div>
            ...
            _html.push(`
                ...
                <div class="text set">
                    ${showFixedNumber && isDetailSplit
                        ...
                        : ''}
                    ${specialtype === 'E06' || specialtype === 'E03' || specialtype === 'E05' ? '' : (
                        `<div class="set-item">
        ...
        _html.push(`
            ${
                specialtype === 'E06' ? (
                    `<div class="text">
                        <div class="name requiredFields">不动产地址</div>
                        <div class="value">
                            <input class="searchInput" disabled type="text" name="simpleaddress_name" maxlength="100" value="${simpleaddress_name}${detailaddress}">
                        </div>
                    </div>
                    <div class="text">
                        <div class="name requiredFields">租赁期起止</div>
                        <div class="value">
                            <input class="searchInput" disabled type="text" name="startleasedate" maxlength="100" value="${startleasedate.substring(0, 10)}~${endleasedate.substring(0, 10)}">
                        </div>
                    </div>
                    <div class="box">
                        <div class="text">
                            <div class="name requiredFields">跨地（市）标志</div>
                            <div class="value">
                                <select class="searchInput" disabled name="redreason">
                                    <option value="" ${crosscitysign === '1' ? 'selected' : ''}>是</option>
                                    <option value="1" ${crosscitysign !== '1' ? 'selected' : ''}>否</option>
                                </select>
                            </div>
                        </div>
                        <div class="text">
                            <div class="name requiredFields">产权证书/不动产权证号</div>
                            <div class="value">
                                <input class="searchInput" disabled type="text" name="estateid" maxlength="100" value="${estateid}">
                            </div>
                        </div>
                    </div>`
                ) : ''
            ...
            ${
                specialtype === 'E03' ? (
                    `<div class="text">
                        <div class="name requiredFields">土地增值税项目编号</div>
                        <div class="value">
                            <input class="searchInput" disabled type="text" name="landtaxno" maxlength="100" value="${landtaxno}">
                        </div>
                    </div>
                    <div class="box">
                        <div class="text">
                            <div class="name requiredFields">建筑项目名称</div>
                            <div class="value">
                                <input class="searchInput" disabled type="text" name="buildingname" maxlength="100" value="${buildingname}">
                            </div>
                        </div>
                        <div class="text">
                            <div class="name requiredFields">跨地（市）标志</div>
                    ...
                    <div class="text">
                        <div class="name requiredFields">建筑服务发生地</div>
                        <div class="value">
                            <input class="searchInput" disabled type="text" name="simpleaddress_name" maxlength="100" value="${simpleaddress_name}${detailaddress}">
                        </div>
                    </div>`
            ...
            ${specialtype === 'E05' && isMulbuyer ? (
                `<div class="text">
                    <div class="name requiredFields">共同购买方信息</div>
                    <div id="imc_invoiceWorkbench_treatment_invoice_buyername${id}" style="width: 76%"></div>
                </div>`
            ) : ''}
            ${
                specialtype === 'E05' ? (
                    `<div class="text">
                        <div class="name requiredFields">不动产信息</div>
                        <div id="imc_invoiceWorkbench_treatment_invoice_statesales${id}" style="width: 76%"></div>
                    </div>`
                ) : ''
            }
        `)
    ...
    function updateTreatmentInvoiceTableByIndex(invoiceIndex, setScrollTop = false) {
        ...
        fpyTable.init({
            ...
            scroll: items.length > 8 ? { h: 208, x: 668 } : {},
            dataSource: items,
            headerColor: '#666',
            headerBgColor: '#eee',
            rowHeight: 26,
    ...
    function updateGoodsTransportTable(invoiceIndex, setScrollTop = false) {
        ...
        let tableDom;
        if (setScrollTop) {
            tableDom = $(`#imc_invoiceWorkbench_treatment_goodsTransport${id}`);
            scrollTop = tableDom.find('.workbench-scroll').scrollTop();
        }
        fpyTable.init({
            id: `imc_invoiceWorkbench_treatment_goodsTransport${id}`,
            rowKey,
            columns,
    ...
    function updateGoodsTravelTable(invoiceIndex, setScrollTop = false) {
        ...
        let tableDom;
        if (setScrollTop) {
            tableDom = $(`#imc_invoiceWorkbench_treatment_goodsTravel${id}`);
            scrollTop = tableDom.find('.workbench-scroll').scrollTop();
        }
        fpyTable.init({
            id: `imc_invoiceWorkbench_treatment_goodsTravel${id}`,
            rowKey,
            columns,
    ...
    function updateGoodsCollectionShipTable(invoiceIndex, setScrollTop = false) {
        ...
        let tableDom;
        if (setScrollTop) {
            tableDom = $(`#imc_invoiceWorkbench_treatment_goodsCollectionShip${id}`);
            scrollTop = tableDom.find('.workbench-scroll').scrollTop();
        }
        
        fpyTable.init({
            id: `imc_invoiceWorkbench_treatment_goodsCollectionShip${id}`,
            rowKey,
            columns,
    ...
    function getTreatmentInvoiceTotalHtmlByIndex(invoiceIndex) {
        const { id: billId } = treatmentShowBillList[treatmentIndex];
        const { totalNum = 0, invoiceamount, totaltax, totalamount } = treatmentInvoiceList[billId][invoiceIndex];
        const _html = `
    ...
    function updateTreatmentSaleestate(invoiceIndex, name, value, itemIndex, maxlength) {
        ...
        if (name === "cobuyer" && value && !/^[\u4e00-\u9fa5a-zA-Z]+$/.test(value)) {
            ...
            $('#imc_invoiceWorkbench_treatment_invoice_buyername' + invoiceList[invoiceIndex].id).find('input[name=cobuyer]').eq(itemIndex).val(value);
            return;
        }
        ...
        if (name === "cobuyer" || name === "cobuyercardno" || name === "cobuyercardtype") {
            if (name === "cobuyercardtype") {
                ...
                $('#imc_invoiceWorkbench_treatment_invoice_buyername' + invoiceList[invoiceIndex].id).find('input[name=cobuyercardno]').eq(itemIndex).val(cacheCardNo);
            }
    ...
    function showPreviewInvoice() {
        ...
        // if (isSpecialE06) {
        //     $preview_bz.text(
        //         `不动产地址：${obj.simpleaddress_name}${obj.detailaddress}\r\n租赁期起止：${obj.startleasedate.substring(0, 10)}~${obj.endleasedate.substring(0, 10)}\r\n跨地（市）标志：${obj.crosscitysign === '1' ? '是' : '否'}\r\n${obj.remark}`
        //     )
        // } else if (isSpecialE03) {
        //     $preview_bz.text(
        //         `土地增值税项目编号：${obj.landtaxno}\r\n跨地（市）标志：${obj.crosscitysign === '1' ? '是' : '否'}\r\n${obj.remark}`
        //     )
        // }  else if (isSpecialE05) {
        //     $preview_bz.text(
        //         `核定计税价格：${obj.approvedprice}\r\n实际成交含税金额：${obj.actualturnover}\r\n不动产单元代码/网签合同备案编号：${obj.estatecode}\r\n土地增值税项目编号：${obj.landtaxno}\r\n跨地（市）标志：${obj.crosscitysign === '1' ? '是' : '否'}\r\n不动产地址：${obj.simpleaddress_name}${obj.detailaddress}\r\n${obj.remark}`
        //     )
        // } else {
        ...
        for (let i = 0; i < obj.items.length; i++) {
            ...
            let curAeaunitE05 = '';
            if (isSpecialE05) {
                const curSeqE05 = parseInt(cur.seq) + 1;
                const curE05Ary = (obj.estatesales || []).filter(item => parseInt(item.seq) === curSeqE05);
                console.log(curE05Ary, '---curE05Ary-1---')
                const curE05Obj = curE05Ary[0] || {};
                curEstateidE05 = curE05Obj.saleestateid;
                curAeaunitE05 = curE05Obj.saleunit;
            }
            _html.push(`<div class="line">
                ...
                ${
                    isSpecialE06 ? (
                        ...
                        `<div class="item truncateText" title="${curEstateidE05}">${curEstateidE05}</div>
                        <div class="item truncateText" title="${curAeaunitE05}">${curAeaunitE05}</div>`
    ...
    // 开票申请单-table选择
    // $bill_table.on('input', 'input', function () {
    //     const type = $(this).attr('data-type');
    //     const checked = $(this).is(':checked');
    //     const index = $(this).attr('data-index');
    //     if (type === 'all') {
    //         if (checked) {
    //             selectedRowKeys = billList.map(o => o[rowKey]);
    //         } else {
    //             selectedRowKeys = [];
    //         }
    //         $bill_table.find('input').prop('checked', checked);
    //     } else {
    //         if (checked) {
    //             selectedRowKeys.push(billList[index][rowKey]);
    //         } else {
    //             const cur = selectedRowKeys.indexOf(billList[index][rowKey]);
    //             selectedRowKeys.splice(cur, 1);
    //         }
    //         $bill_table.find('input[data-type=all]').prop('checked', selectedRowKeys.length === billList.length);
    //     }
    // });

    // 开票申请单-table行点击
    // $bill_table.on('click', 'tbody tr', function (e) {
    //     const dom = $(this).find('input');
    //     const index = dom.attr('data-index');
    //     const checked = dom.is(':checked');
    //     if ($(e.target).is('a')) {
    //         const key = $(e.target).attr('data-key');
    //         if (checked) selectedRowKeys = selectedRowKeys.filter(o => o !== key);
    //         billList.splice(index, 1);
    //         showBillTable(billList);
    //         updateBillHeader();
    ...
    $bill.find('.footer').on('click', '.next', function() {
        ...
        if (!ids.length) {
            // common/tip
            model.invoke("common/tip", '开票申请单不能为空！');
            return;
        }
    ...
    $treatment_bill.on('mouseup', '.item', function(e) {
        ...
        if ($(e.target).hasClass('sure')) {
            ...
            if (!disabled) {
                const dom = $('#imc_invoiceWorkbench_treatment_bill' + billId);
                // 复选框
                let keys = [];
                if (type === 'all') {
                    if (checked) {
    ...
    $treatment_solutions.on('click', '.solutions-item', function() {
        ...
        if (!disabled) {
            ...
            model.invoke("process/changeSplitRule", request);
            // }
        }
    });
    ...
    $treatment_invoice.on('dblclick', 'input[type="text"]', function() {
        ...
        if (name === 'startplace' || name === 'endplace') {
            model.invoke("process/queryPlace", {
                invoiceIndex,
                itemIndex,
                name
            });
    ...
    function treatmentInvoiceSearch(that) {
        ...
        if (!disabled) {
            let request = {
                ...
                "buyerproperty" : obj.buyerproperty,

                invoiceContent: obj.invoiceContent || '1'
            }
            if (name === "buyername") {
                // 目前后端未实现
                model.invoke("process/openbuyerlist", { billId, index: treatmentIndex });
