...
// 过滤特殊字符 除了中英文小括号空格
String.method('fpy_filterSpecialChar', function() {
    return this.replace(/[^a-zA-Z0-9()（）\u4e00-\u9fa5\s]/g, '');
});

// 仅数字
String.method('fpy_onlyNum', function() {
    return this.replace(/\D/g, '');
});

// 大写字母数字 税号
String.method('fpy_toUpperLetterNum', function() {
    return this.replace(/[^a-zA-Z0-9]/g, '').toUpperCase();
});

// 邮箱输入
String.method('fpy_onlyEmali', function() {
    // eslint-disable-next-line
    return this.replace(/[^a-zA-Z0-9_\.-@\;]/g, '');
});

// 邮箱校验
String.method('fpy_isEmail', function() {
    // eslint-disable-next-line
    return (/^([a-zA-Z0-9_\.-]+)@([a-zA-Z0-9\.-]+)\.([a-zA-Z\.]{2,6})$/.test(this));
});

// 手机校验
String.method('fpy_isPhone', function() {
...
function fpy_getUUId() {
    ...
    const uuid = 'xxxxxxxxxxxxxxyxxxxxxxxxxxxxxxxx'.replace(/[xy]/g, function(c) {
...
// 单价保留13位小数
function fpy_toFixedTwoOrMore_dj(val) {
    const xsd = val.toString().split('.');
    if (!xsd[1]) {
        return val + '.00';
    } else if (xsd[1].length === 1) {
        return val + '0';
    } else if (xsd[1].length > 13) {
        return xsd[0] + '.' + xsd[1].substring(0, 13);
    } else {
        return val;
    }
}
