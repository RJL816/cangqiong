(function(KDApi, $){
    ...
    Condition.prototype = {
		...
		handleDirective:function(object, method, args){	
			...
			if(model && model.dom && model.dom.innerHTML && PermConditionCodeMirror){
    ...
    var initFunc = function(model, props) {
		...
		KDApi.loadFile(['../rule/css/pingtai.css','../rule/css/jquery-ui.min.css','./css/permcondition.css','../rule/css/selectivity-jquery.css','./css/codemirror.css', '../rule/css/customStyle.css'], model, function(){
				...
				KDApi.loadFile('../rule/js/selectivity-jquery.js', model, function(){
					KDApi.loadFile('./js/codemirror.js', model, function() {
						KDApi.loadFile('./js/customruleMode.js', model, function() {
							...
							KDApi.getTemplateStringByFilePath('./html/permcondition.html', model, {
								text: text,
								...
								emptyDescription:KDApi.getLangMsg(model, 'emptyDescription'),
								showControl:KDApi.getLangMsg(model, 'showControl')
							}).then(function(data) {
    ...
    var updateFunc = function(model, data) {	
		...
		try{
			if(data.display != undefined && !data.display){
				$('.permconditionControlDiv', model.dom).hide();
			}else{
				$('.permconditionControlDiv', model.dom).show();
			}
			if(model.initing == "true"){
				model._defaultRows = data.defaultRows && data.defaultRows
				model._showDeleteOnlyOneRow = data.showDeleteOnlyOneRow && data.showDeleteOnlyOneRow;
				model._hideButton = data.hideButton && data.hideButton;
				// model._comparisonOptMap = data.comparisonOpt && JSON.parse(JSON.stringify(data.comparisonOpt).replaceAll("name","text").replaceAll("value","id"));
				model._showValueType = false;
				hideButton(model);
	...
	var changePageState = function(model, pageState){
		...
		if(pageState.toLocaleLowerCase() == "view"){//查看态
			...
			$(".ruleCondition tr", model.dom).each(function(){
				showValueSearchView(model, $(this));
			});
			// 隐藏日期图标
	...
	var showContent = function(model){
		$(".emptyControl", model.dom).hide();
		$(".permconditionControl", model.dom).show();
	}
	...
	var initControl = function(model, data){
		...
		$radio = $("input[name='conditionType" + model.pageId + "']", model.dom).parent();
		$radio.removeClass('kdfont-liebiaodanxuankuangxuanzhong').removeClass('theme-fc');
		...
		$.each($radio, function(){
			// BT-01652764 既没有选中也没有未选中样式时，增加未选中样式
			if(!$(this).hasClass('kdfont-danxuankuang-weixuan') && !$(this).hasClass('kdfont-liebiaodanxuankuangxuanzhong')){
				$(this).addClass('kdfont-danxuankuang-weixuan')
			}
		})
	...
	var getBaseDataNumber = function(model, $tr){
		return $tr.find(".conditionParam").selectivity('data') && $tr.find(".conditionParam").selectivity('data').entityNumber;
	}
	...
	var isMulti = function(model, $tr){
		return ($tr.find(".comparisonOpt").selectivity('data') && $tr.find(".comparisonOpt").selectivity('data').selecttype) === "2";
	}
	
	var getComparisonOptName = function(model, $tr){
		return $tr.find(".comparisonOpt").selectivity('data') && $tr.find(".comparisonOpt").selectivity('data').text
	}
	...
	var getParamFilter = function(model, $tr){
		return $tr.find(".conditionParam").selectivity('data') && $tr.find(".conditionParam").selectivity('data').filters;
	}
	 ...
	 *   enum:不影响(只有等于/不等于)
	 */
	var comparisonOptChangeEvent = function(model, $tr){
		var mysel = $tr.find(".comparisonOpt");
		var newValueType = mysel.selectivity('data').selecttype;
		...
		if(newValueType === "0"){
			...
			if(getConditionParamType(model, $tr) == "dynamicObject" || isEnum4ParamType(model, $tr)){
				...
				if(newvalue == "containsub"){// 包含下级只能选值
					if($tr.find(".valueType .selectivity-single-select", model.dom).length > 0){
				...
				//多选切换成单选时，清空value值
				if((oldValueType == "2") && (newValueType == "1")){
					clearValue(model, $tr);
				}				
			}			
		}	
	...
	var showValueSearch = function(model, $tr){
		...
		$(".conditionValueSearchView", model.dom).hide();
		...
		if((paramType == 'dynamicObject' && valueType == valueTypeMap.value) || valueType == valueTypeMap.target){
			...
			$(".conditionValueSearch", $tr).off('click').on('click', function(){
				...
				if(valueType == valueTypeMap.value){//值			
					// 传已选择的值	
					var entityNumber = getConditionParamEntityNumber(model, $tr) || getConditionParamValue(model, $tr);
					...
					model.invokeCustomMethod("openParamF7", {entityNumber : entityNumber, index : index, isMulti : isMulti(model, $tr), value : value, filters : getParamFilter(model, $tr)});
	...
	// 预览方法
	var showValueSearchView = function(model, $tr){
		console.log(model)
		//显示搜索框（参数类型为object，值类型为值；值类型为指标）
		var paramType = getConditionParamType(model, $tr);
		var valueType = getValueTypeValue(model, $tr);
		if(paramType == 'dynamicObject' && valueType == valueTypeMap.value){
			$tr.find(".conditionValueSearchView").show();
			$(".conditionValueSearchView", model.dom).off('click').on('click', function(){
				var $tr = $(this).parents("tr");
				var index = $tr.index();//从1开始
				//var valueType = $tr.find(".valueType").val();
				var valueType = getValueTypeValue(model, $tr);
				if(valueType == valueTypeMap.value){//值
					...
					model.invokeCustomMethod("openViewParamF7", {entityNumber : entityNumber, index : index, isMulti : isMulti(model, $tr), value : value, filters : getParamFilter(model, $tr)});
				}
			});
		}else{
			$tr.find(".conditionValueSearchView").hide();
			$tr.find(".conditionValueSearchView").off('click');
		}
	}
    ...
    var initEvent = function(model, props){
		$('.showControl', model.dom).click(function(){
			if (model.pageState && model.pageState.toLocaleLowerCase() == "view") {
				return;
			}
		...
		$(model.dom).on('click', '.ruleCondition .copy', function(){
			...
			// 值类型
			var paramType = getConditionParamType(model, $tr);
			var valueType = getValueTypeValue(model, $tr);//$tr.find(".valueType").selectivity('value');
			...
			createComparisonOptControl(model, $tr2, $tr.find(".comparisonOpt").selectivity('value'));
		...
		$('.expressView', model.dom).click(function(){
			var optMap = {};
			$.each(model._comparisonOptMap, function(type, comparisonOpt){
				$.each(comparisonOpt, function(i, opt){
					optMap[opt.id]=opt.text;
				})
			});
			model.invokeCustomMethod("openExpressView", {comparisonOpt:optMap});
        });
		...
		if(conditionList && conditionList.length > 0){
			...
			$.each(conditionList, function(i, conditionObj){
				try{				
					...
					// 如果预置了过滤条件
					if(conditionObj.filters){
						$tr.find(".conditionParam").selectivity('data').filters = conditionObj.filters;
					}
					
					$tr.find(".displayValue").val(conditionObj.displayValue)
					$tr.find(".value").val(conditionObj.value)
				}catch(err){
					// catch异常，防止其中一个条件影响全部条件
					console.log(err);
				}
			});
		}
	...
	//创建比较符控件
	var createComparisonOptControl = function(model, $tr, defaultValue){
		...
		$('.comparisonOpt', dom).off('change').on('change', function(e){
			//copy时，需要触发比较符改变事件，为空或不为空时，禁用参数和结果
			if(model.initing == "true" || e.target.className != "comparisonOpt" || "" === $(this).selectivity("value")){
				return;
			}
			
			comparisonOptChangeEvent(model, $(this).parents("tr"));
			conditionJsonChange(model);
		});
		
		$('.comparisonOpt', dom).selectivity("value", defaultValue ? defaultValue : model._comparisonOptMap[type][0].id);
	}
	...
	var createValueTypeControl = function(model, $tr, notDefault, paramType){
		...
		$('.valueType', dom).off('change').on('change', function(e){
			...
			var valueType = getValueTypeValue(model, $tr);//$(this).selectivity('value');
			if(valueType == valueTypeMap.param){//参数
				showParamSelectDiv(model, $tr);
			}else if(valueType == valueTypeMap.value){//值
				showControlAndSetValue(model, $tr);
	...
	var getConditionList = function(model){
		...
		$(".ruleCondition tr", model.dom).each(function(i){
			...
			conditionObj.baseDataNumber = getBaseDataNumber(model, $(this));
			if(conditionObj.paramType == "date" && conditionObj.valueType == "1"){
				conditionObj.valueDateFormat = getValueDateFormat(model, $(this));
			}
			
			//if(isAdminOrg4ParamType(model, $(this)) && "containsub" == conditionObj.operators){
	...
	var createConditionExpress = function (model, data) {
		...
		model.swcEditor = PermConditionCodeMirror.fromTextArea($('.conditionExpress', model.dom)[0], {
	...
	var saveEditor = function(model, value, isInit){
		...
		if(!getEditorValue(model) || (!isInit && model.onlyUpdateWeb !== "true")){
    ...
    var setEditorValue = function(model, value, isInit) {
		if(model.swcEditor){
			if($('.permconditionControl', model.dom).is(":visible")){
    ...
    KDApi.register('permcondition', Condition, {isMulLang: true})
})(window.KDApi, jQuery)
