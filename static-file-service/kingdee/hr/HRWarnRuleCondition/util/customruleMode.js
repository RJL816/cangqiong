(function (mod) {
  ...
  { mod(window.RuleConditionCodeMirror) }
})(function (CodeMirror) {
  'use strict'

	var propertyKeyWord = ['并且', '或者', '(', ')','AND', 'OR'];
	...
	RuleConditionCodeMirror.defineMIME('text/customRuleCondion', {
		...
		name: 'customRuleCondion'
	})
		
  CodeMirror.defineMode('customRuleCondion', function(config, parserConfig){
    if (!parserConfig.propertyKeyWord) {
		parserConfig = CodeMirror.resolveMode('text/customRuleCondion')
    }
