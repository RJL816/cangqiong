...
(function () {
    ...
    Colorpicker.prototype = {
        init: function init(opt) {
            ...
            //初始化tips：
            var dom_tipsIcon = this.Opt.model.dom.querySelector(".wtc_colorPicker-text-tips");
            ...
            if (this.Opt.status !== 'VIEW') {
                ...
                this.elem_colorPalette.addEventListener("click", function (e) {
                    if (e.target.tagName.toLocaleLowerCase() === "p") {
                        var _color = $(e.target).attr('bgc');
                        _this.setColorByInput(_color);
                        _this.hide();
                        // model.invoke，用于给后端发送请求，第一个参数是事件名，可自定义；第二个参数是发送给后端的数据，可以是任意类型
                        _this.Opt.model.invoke('click', _color);
                    }
                }, false);
            } else {
        ...
        // 获取颜色item对应的dom语句：
        getPaletteColorsItem: function getPaletteColorsItem() {
            var str = '';
            var palette = ["#276FF5", "#389E0D", "#701DF0", "#16B8B1", "#FF5F1F", "#FF3B72", "#16B0F1", "#FF991C", "#8590A6"];
            // let palette = [  // 旧版班次颜色
        ...
        // 设置选中的颜色：
        setColorByInput: function setColorByInput(color) {
