(function(KDApi, $){
	...
	var isBdNumber = function(model, $tr){
		return $tr.find(".conditionParam").selectivity('data') && $tr.find(".conditionParam").selectivity('data').id.endsWith(".number");
	}
    ...
    var initEvent = function(model, props){
		...
		$('.addCondition', model.dom).on('click', function(){
            addConditionRows(model, 1);
        })
		
		$('.expressView', model.dom).on('click', function(){
			model.invokeCustomMethod("openExpressView");
        });
		
		$('.and', model.dom).on('click', function(){
			insertEditorValue(model, DIS_LOGIC_AND);
        })
		
		$('.or', model.dom).on('click', function(){
            insertEditorValue(model, DIS_LOGIC_OR);
        })
		
		$('.leftBracket', model.dom).on('click', function(){
            insertEditorValue(model, LEFT_BRACKET);
        })
		
		$('.rightBracket', model.dom).on('click', function(){
	...
	var updateTextValue = function(model, data){
		...
		if(isBdNumber(model, $tr)){
			$tr.find(".valueText").val(data.numberList || data.idList)
			$tr.find(".displayValue").val(data.numberList || data.idList);
		}else{
    ...
    KDApi.register('rulecondition', Condition, {isMulLang: true})
})(window.KDApi, jQuery)
