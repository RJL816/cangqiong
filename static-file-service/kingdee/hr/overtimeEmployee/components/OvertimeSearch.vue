<template>
  <div class="overtime_search">
    <div class="search_box" v-if="searchParam.searchType!=='0'">
      <el-input
          placeholder="请搜索档案编号/姓名/工号"
      ...
      <div class="clear" @click="close()">取消</div>
    </div>
    <div style="display: flex;flex-direction: column;align-items: stretch;flex-grow: 1;flex-shrink: 1;height: 10px">
      <div class="search_content" @scroll="handleScroll" v-if="resultList.length">
        <div class="search_item" v-for="(item,index) in resultList" :key="index+'search'"
             @click="selectItem(item)">
          <img :src="item.headImg ? item.headImg : 'images/pc/emotion/default_person_82_82.png?v=1.0'" alt="">
          <div class="item">
            <div class="person">
              <div class="name" v-html="highlightHTML(item.personName)">{{ item.personName }}</div>
              <span>(</span>
              <div class="number" v-html="highlightHTML(item.personNumber)">{{ item.personNumber }}</div>
              <span>)</span>
            </div>
            <div class="department">{{ item.adminorgName }}</div>
          </div>
        </div>
      </div>
      <div class="noData" v-if="!resultList.length && !searchParam.searchContent ">
...
export default {
  props: {
    searchResult: Array,
    cusParam_searchParam: Object,
    cusParam_searchPage: Number
  },
  computed: {},
  watch: {
    searchResult(resultObj) {
      if (this.searchParam.pageNum <= this.totalPage) {
       ...
       this.resultList = this.resultList.concat(resultObj);
    ...
    cusParam_searchParam: function (obj) {
      this.searchParam = obj;
      if (this.searchParam.searchType === '0') {
        this.searchParam.searchContent = '本人';
      }
    },
    cusParam_searchPage: function (obj) {
      this.totalPage = obj;
    }
  ...
  methods: {
    ...
    searchInit() {
        ...
        this.isEnd = false;
        this.isfresh = true;
        if(this.searchParam.searchContent == ''){
          this.resultList = [];
          return;
        }
        this.search();
    ...
    selectItem(item) {
      this.invoke("employee_select", item);
    },
      ...
      this.timer = setTimeout(() => {
        ...
        if (innerHeight >= (outerHeight - scrollTop)) {
          if (!this.isEnd) {
            ...
            if (this.searchParam.pageNum <= this.cusParam_searchPage) {
              this.search()  //请求接口的函数
            }
          }
        }
