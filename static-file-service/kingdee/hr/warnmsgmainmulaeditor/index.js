(function(KDApi, $){
	...
	var initFunc = function(model, props) {
        ...
        KDApi.loadFile(['../warnmsgtitlemulaeditor/js/codemirror.js','../warnmsgtitlemulaeditor/css/codemirror.css'], model, function(){
			KDApi.loadFile('./js/mainTargetMode.js', model, function () {
				KDApi.loadFile('../warnmsgtitlemulaeditor/css/warnmsg.css', model, function() {
					...
					KDApi.getTemplateStringByFilePath('./html/warnmsgmain.html', model, {
						text: text,
    ...
    var initEvent = function(model, props){
		// 表达式编辑框数据改变时需要向后台传数据
		$(model.dom).on('change', '#msgMainText', function(){
			model.invoke('returnMsgMainText', $("#msgMainText").val());
		});
	
		$(model.dom).on('mouseup', '#msgMainDiv', function() {
			model.invoke('returnCurFocusText', 'msgmain');
		});
	}
	...
	function insertAtCursor(model, text) {
	    var expressionField = window.document.getElementById('msgMainText')
	...
	var createExpression = function(model, props, createMethod) {
		...
		TargetExpressionCodeMirror.defineMIME('text/msgMainTarget', {
    ...
    KDApi.register('warnmsgmainmulaeditor', Expression, {isMulLang: true})
})(window.KDApi, jQuery)