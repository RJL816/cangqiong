...
var RESULT_ITEM_WRAP = '.selectivity-result-label-wrap';
...
function SelectivityDropdown(selectivity, options) {
    ...
    events['click ' + RESULT_ITEM_WRAP] = this._resultWrapClick;
...
assign(SelectivityDropdown.prototype, {
    ...
    search: function(term) {
        ...
        if (this.options.items) {
			...
			var resultsHtml = this.selectivity.template('noResults', { term: "" });
     ...
     * @private
     */
    _resultWrapClick: function(event) {
        event.preventDefault()
        event.stopPropagation()
        let {target} = event
        if (!$(target, this.el).hasClass('selectivity-result-label-wrap')) {
            target = target.parentElement
        }
        // clickIconopen clickIconclose
        let iconDom = target.children[0]
        // 收起第一最近的子集
        let childDom = $(target).nextAll('div.selectivity-result-children:first').get(0)
        // childDom.setAttribute('isShow', 'true')
        let isShow = childDom.getAttribute('isShow')
        switch (isShow) {
            case "true":
                childDom.setAttribute('isShow', 'false')
                $(iconDom).removeClass('clickIconopen').addClass('clickIconclose')
                $(childDom).hide()
                break
            case "false":
                childDom.setAttribute('isShow', "true")
                $(iconDom).removeClass('clickIconclose').addClass('clickIconopen')
                $(childDom).show()
        }
        stopPropagation(event)
    },
...
var callSuper = Selectivity.inherits(SingleInput, Selectivity, {
    ...
    close: function(options) {
		var openTime =  $(".xialajiantou.active", this.el).attr('openTime');
		// 规则测试页面自动关闭下拉框问题
		//console.log("time1:" + new Date().getTime());
		//console.log("time2:" + openTime);
		//console.log("time3:" + (new Date().getTime() - openTime));
		if(new Date().getTime() - openTime < 1000){//小于1s，不关闭
			return;
		}
		...
		$(".xialajiantou", this.el).removeClass('xiangshangjiantou');
		$(".xialajiantou", this.el).removeClass('theme-fc');
		$(".xialajiantou", this.el).removeClass('active');
...
module.exports = Selectivity.Locale = {
    ...
    loadMore: localStorage.getItem("loadMore"),
    noResults: localStorage.getItem("noResults"),

    ajaxError: function(term) {
        if (term) {
            return '无法获取<b>' + escape(term) + '</b>的参数';
        } else {
            return '无法获取参数';
        }
    },
    ...
    needMoreCharacters: function(numCharacters) {
        return '请出入更多' + numCharacters + '字符去查询';
    },

    noResultsForTerm: function(term) {
        return localStorage.getItem("noResultsForTerm").replace('%s', escape(term));
    }
};

module.exports_en = Selectivity.Locale_en = {
    loading: 'Loading...',
    loadMore: 'Load more...',
...
assign(Selectivity.prototype, {
    ...
    open: function() {
		...
		$(".xialajiantou", this.el).addClass('xiangshangjiantou')
		$(".xialajiantou", this.el).addClass('active');		
		$(".xialajiantou", this.el).attr('openTime', new Date().getTime());
    ...
    _mouseenter: function() {
       // toggleClass(this.el, 't', true);	   
	   $(".xialajiantou",this.el).addClass('theme-fc')
    }
});
    ...
    if (Selectivity.isValidId(item)) {
        ...
        if (item.children) {
			if(typeof item.children == "string"){
				item.children = Selectivity.processItems(JSON.parse(item.children));
			}else{
...
Selectivity.Templates = {
    ...
    resultChildren: function(options) {
        let {level} = options
        let classList = level == "2" ? "selectivity-result-children-level2" : level == "3" ? "selectivity-result-children-level3" : ""
        return '<div isShow="true" class="selectivity-result-children '+classList+'">' + options.childrenHtml + '</div>';
    },
    ...
    resultItem: function(options) {
        let {level} = options
        let classList = level == "2" ? "sselectivity-result-item-level2" : level == "3" ? "selectivity-result-item-level3" : ""
        return (
            '<div class="'+classList+' selectivity-result-item' +
            (options.disabled ? ' disabled' : '') +
            '"' +
            ' data-item-id="' +
            escape(options.id) +
            '"><span class="selectivity-result-item-span">' +
            escape(options.text) +
            (options.submenu
    ...
    resultLabel: function(options) {
        // HR基础平台规则引擎最多只会3层
        let {level} = options
        let classList = level == '2' ? "selectivity-result-label-wrap-leve2" : level == '3' ?  ' selectivity-result-label-wrap-leve3' : ''
        return '<div level='+level+' class ="selectivity-result-label-wrap '+classList+'"><i class="clickIconopen"></i><div class="selectivity-result-label">' + escape(options.text) + '</div></div>'
    ...
    singleSelectInput: function(options) {
        return (
            ...
            '>' +
            '<div class="selectivity-single-result-container"></div></div>' +
            '<div class="Select_iconBox_1WJ3"><div class="xialajiantou kdfont kdfont-xiala Select_select-icon-default_3JG_" style="float: right;padding: 5px 0;"></div></div>'
        );
    },
        ...
        return (
            ...
            (options.removable
                ? '<a class="selectivity-single-selected-item-remove">' +
                  ...
                  '</a>'
                : '') +
				//options.text 改成options.name
            escape(options.name || options.text) +
            '</span>'
        );
    },
