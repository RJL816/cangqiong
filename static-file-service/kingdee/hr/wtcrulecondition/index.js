(function (KDApi, $) {
	...
	Condition.prototype = {
		...
		handleDirective: function (object, method, args) {
			...
			if (model && model.dom && model.dom.innerHTML && RuleConditionCodeMirror) {
	...
	function loadFileReWrite(model, path, callBack) {
		let { schemaId, isvId, moduleId } = model;
		if (Array.isArray(path)) {
			path = path.map(pathItem => pathItem.filePath = `./${isvId}/${moduleId}/${schemaId}/${pathItem.filePath}?version=${new Date().getTime()}`);
		} else {
			path.filePath = `./${isvId}/${moduleId}/${schemaId}/${path.filePath}?version=${new Date().getTime()}`;
		}
		loadFileFunc(path, typeof callBack == "function" && callBack);
	}
	function loadFileFunc(path, callBack) {
		if (Array.isArray(path)) {
			Promise.all(path.map(item => {
				loadFileToHead(item);
			})).then(function () {
				callBack();
			})
		} else {
			loadFileToHead(path, callBack);
		}
	}
	/**
	 * path: {id: '', filePath: '', type: 'css/js'}
	*/
	function loadFileToHead(path, callBack) {
		return new Promise(function (resolve, reject) {
			if (!path) return;
			let headDom = document.head || document.getElementsByTagName('head')[0];
			let fileDom = '';
			if (path.type == 'css') {
				fileDom = document.createElement('link');
				fileDom.rel = 'stylesheet';
				fileDom.href = path.filePath;
				fileDom.id = path.id;
			} else { // load js
				fileDom = document.createElement('script');
				fileDom.type = 'text/javascript';
				fileDom.src = path.filePath;
				fileDom.id = path.id;
			}
			headDom.appendChild(fileDom);
			fileDom.onload = function (val) {
				if (callBack) {
					callBack();
				} else {
					resolve();
	...
	var initFunc = function (model, props) {
		...
		KDApi.loadFile(['./css/pingtai.css','./css/jquery-ui.min.css','./css/condition.css','./css/selectivity-jquery.css','./css/codemirror.css', './css/customStyle.css'], model, function(){
			KDApi.loadFile('./js/jquery-ui.min.js', model, function(){
				loadFileReWrite(model, {id: 'selectivityJS', filePath: './js/selectivity-jquery.js', type: 'js'}, function () {
					KDApi.loadFile('./js/codemirror.js', model, function() {
						KDApi.loadFile('./js/customruleMode.js', model, function() {
							...
							KDApi.getTemplateStringByFilePath('./html/condition.html', model, {
								...
								// 初始化加载数据
								// initAndLoadData(model, props);
								// 先藏添加条件按钮，后面在changePageState中判断是否要显示，避免闪屏
								model._hideButton = true;
								hideButton(model);
								// 先禁用条件逻辑选项，后面在changePageState中判断是否要启用，避免闪屏
	...
	/**
	 * 初始化加载数据
	 * @param {*} props 自定义控件属性信息
	 */
	var initAndLoadData = function (model, props) {
		let configItems = props.configItems;
		// 请求参数
		let params = {};
		for (var ii = 0; ii < configItems.length; ii++) {
			if (configItems[ii]) {
				params[configItems[ii].key] = configItems[ii].value;
			}
		}
		// 请求初始化
		console.info(params);
		model.invoke('init', params);
	}
	...
	var updateFunc = function (model, data) {
		...
		try {
			if (model.initing == "true") {
				...
				// 默认不展示值类型列
				model._showValueType = data.showValueType == undefined ? false : data.showValueType;
				model.ruleEngine = data.ruleEngine;
				model._valueTypeIsShow = data.valueTypeIsShow;
				// hideButton(model);
	...
	var changePageState = function (model, pageState) {
		...
		if (pageState.toLocaleLowerCase() == "view") {//查看态
			...
			for (let i = 0; i < radios.length; i++) {
				let temp = $(radios[i]);
				if (temp.hasClass('kd-hover-color')) {
					temp.removeClass('kd-hover-color');
				}				
	...
	var enableInputValue = function (model, $tr) {
		...
		$(".valueDateDiv", $tr).off('click').on('click', function () {
			...
			model.invoke("setDateFormat", { dateFormat: getDateFormat(model, $tr),paramValue: $tr.find(".value").val() });
		});
	...
	var getConditionParamUniqueCode = function (model, $tr) {
		return $tr.find(".conditionParam").selectivity('data') && $tr.find(".conditionParam").selectivity('data').uniqueCode;
	}

	var getConditionParamRetrievalWay = function (model, $tr) {
		return $tr.find(".conditionParam").selectivity('data') && $tr.find(".conditionParam").selectivity('data').retrievalWay;
	}
	...
	var comparisonOptChangeEvent = function (model, $tr, value) {
		...
		if ("is_not_null" == value || "is_null" == value || /^[+-]*\dd$/.test(value) || /^[+-]*\dw$/.test(value)
			|| /^[+-]*\dm$/.test(value) || /^[+-]*\dq$/.test(value) || /^[+-]*\dy$/.test(value)
		...
		if ((paramType == 'dynamicObject' && valueType == valueTypeMap.value) || valueType == valueTypeMap.target) {
			...
			$(".conditionValueSearch", $tr).off('click').on('click', function () {
				...
				if (valueType == valueTypeMap.value) {//值			
					...
					model.invoke("openParamF7", { entityNumber: entityNumber, index: index, isMulti: isMulti(model, $tr), value: value });
				} 
			});
	...
	var initEvent = function (model, props) {
         ...
         * 点击参数列，恢复操作列和值列的状态
         */
        $(model.dom).on('click','.conditionParamTd',function(e){
            $(e.target).closest('.conditionControl').find('.comparisonOptTd').find('.selectivity-dropdown').hide();
            $(e.target).closest('.conditionControl').find('.comparisonOptTd').find(".xialajiantou").removeClass("xiangshangjiantou");
            $(e.target).closest('.conditionControl').find('.multiDropDownControl').hide();
            $(e.target).closest('.conditionControl').find('.valueTd').find(".xialajiantou").removeClass("xiangshangjiantou");
        });

        /**
         * 点击操作列，恢复参数列和值列的状态
         */
        $(model.dom).on('click','.comparisonOptTd',function(e){
            $(e.target).closest('.conditionControl').find('.conditionParamTd').find('.selectivity-dropdown').hide();
            $(e.target).closest('.conditionControl').find('.conditionParamTd').find(".xialajiantou").removeClass("xiangshangjiantou");
            $(e.target).closest('.conditionControl').find('.multiDropDownControl').hide();
            $(e.target).closest('.conditionControl').find('.valueTd').find(".xialajiantou").removeClass("xiangshangjiantou");
        });
         ...
         * 点击值列，恢复参数列和操作列的状态
         */
        $(model.dom).on('click','.valueTd',function(e){
            $(e.target).closest('.conditionControl').find('.conditionParamTd').find('.selectivity-dropdown').hide();
            $(e.target).closest('.conditionControl').find('.conditionParamTd').find(".xialajiantou").removeClass("xiangshangjiantou");
            $(e.target).closest('.conditionControl').find('.comparisonOptTd').find('.selectivity-dropdown').hide();
            $(e.target).closest('.conditionControl').find('.comparisonOptTd').find(".xialajiantou").removeClass("xiangshangjiantou");
		...
		$('.expressView', model.dom).click(function () {
			...
			model.invoke("openExpressView", conditionJson);
		});
		...
		//逻辑连接符改变时
		// $("input[type='radio'][name='conditionType" + model.pageId + "']", model.dom).change(function(){
		// 	let selectDom = $('.kdfont-liebiaodanxuankuangxuanzhong', model.dom)
		// 	selectDom.removeClass('kdfont-liebiaodanxuankuangxuanzhong')
		// 	selectDom.removeClass('theme-fc')
		// 	selectDom.addClass('kdfont-danxuankuang-weixuan')
		// 	let changeDom = $("input[name='conditionType" + model.pageId + "']:checked", model.dom)
		// 	var conditionType = changeDom.val();
		// 	changeDom.parent().removeClass('kdfont-danxuankuang-weixuan')
		// 	changeDom.parent().addClass('kdfont-liebiaodanxuankuangxuanzhong')
		// 	changeDom.parent().addClass('theme-fc')
	...
	var setValue = function (model, $element, displayValue, value) {
		...
		if ($element.hasClass("valueTd")) {
			...
			//console.log("赋值出错，无法找到valueTd：" + $element.attr("class"));
			return;
		}
	...
	var getConditionList = function (model) {
		...
		$(".ruleCondition tr", model.dom).each(function (i) {
			...
			// 添加取数项目的唯一标识
			conditionObj.uniqueCode = getConditionParamUniqueCode(model, $(this));
			// 添加取数方式
			conditionObj.retrievalWay = getConditionParamRetrievalWay(model, $(this));
	...
	var saveEditor = function (model, value, isInit) {
		// 如果是初始化，且表达式已经有值了，则不再覆盖表达式的值（延迟设值会导致更新值在初始化之前执行）
	...
	KDApi.register('wtcrulecondition', Condition, { isMulLang: true })
})(window.KDApi, jQuery)
