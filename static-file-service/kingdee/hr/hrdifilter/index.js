    ...
    */
    ...
    var updateFunc = function(model, data) {	
		...
		try{
			...
			if(data.param){
				...
				model._basedataParam = data.basedataParam;
			...
			if(data.date != null && model.dateIndex != -1){
	...
	var changePageState = function(model, pageState){
		...
		if(pageState.toLocaleLowerCase() == "view"){//查看态
			...
			// 取消布尔事件（事件必须放前面，否则影响样式）
			$('.ruleCondition .valueBooleanSelectDiv', model.dom).off('click');
			// 条件编辑禁用，并且修改样式
			...
			$(".ruleCondition input", model.dom).attr("disabled", true).css("color","#808080");				
			...
			// 隐藏下划线高亮
			$(".expressDiv div.kd-hover-color", model.dom).removeClass("kd-field-border-all");			
			// 隐藏下划线高亮
	...
	var initControl = function(model, data){
		...
		if(model.conditionExpressType == null){
			model.conditionExpressType = data.conditionExpressType;
		}
		// 选中值
		let selectDom = $("input[name='conditionType" + model.pageId + "'][value='" + model.conditionExpressType + "']", model.dom)		
	...
	var enableInputValue = function(model, $tr){
		...
		$(".valueDateDiv", $tr).off('click').on('click', function(){
			...
			model.invoke("setDateFormat", {dateFormat : getDateFormat(model, $tr)});
		});
	...
	var showValueSearch = function(model, $tr){
		...
		var paramType = getConditionParamTypeDetail(model, $tr);
		...
		if((paramType == 'dynamicObject' && valueType == valueTypeMap.value) || valueType == valueTypeMap.target){
			...
			$(".conditionValueSearch", $tr).off('click').on('click', function(){
				...
				if(valueType == valueTypeMap.value){//值			
					...
					model.invoke("openParamF7", {entityNumber : entityNumber, index : index, isMulti : isMulti(model, $tr), value : value, operators :  getComparisonOptValue(model,  $tr)});
					...
					model.invoke("openTargetF7", {targetType:"all", paramType : getConditionParamType(model, $tr), index : index, value : $tr.find(".value").val()});
				}
			});
	...
	var dateRelocation = function(model, triggerClick, waitTime){
		...
		dateDom.css({position: 'absolute',left: elemRect.left + 1, top: elemRect.top + 2, width: elemRect.width - 2, display: 'block'});
		dateDom.off('blur').on('blur', 'input',function(){
			closeDate(model);
		});
		var interval = setInterval(function(){
			num++;
    ...
    var initEvent = function(model, props){
		...
		$(model.dom).on('click', '.ConditionTypeRadio', function(e) {
			...
			model.conditionExpressType = conditionType;
	...
	var createValueTypeControl = function(model, $tr, notDefault, paramType){
		...
		$('.valueType', dom).off('change').on('change', function(e){
			...
			updateValueParamItems(model, $tr);
			if(valueType == valueTypeMap.param){//参数
				showParamSelectDiv(model, $tr);
			}else if(valueType == valueTypeMap.value){//值
				showControlAndSetValue(model, $tr);
	...
	var updateValueParamItems = function(model, $tr){
		var dom = $tr || model.dom;
		var typeDetail = getConditionParamTypeDetail(model, $tr);
		var type = getConditionParamType(model, $tr);
		var valueType = getValueTypeValue(model, $tr);
		if(valueType == valueTypeMap.value && (typeDetail == "dynamicObject" || typeDetail == "mul_dynamicObject")){
			$('.valueParam', dom).selectivity({
				allowClear: true,
				items: model._basedataParam[typeDetail] || null
			});
		} else {
			$('.valueParam', dom).selectivity({
				allowClear: true,
	...
	var createValueEnumControl = function(model, $tr, width){
		...
		var enumList = getConditionEnumList(model, $tr);		
	...
	var createConditionExpress = function (model, data) {
		...
		// 先清除，在添加
		$(".conditionExpressDiv div", model.dom).remove();
    ...
    KDApi.register('hrdifilter', Condition, {isMulLang: true})
})(window.KDApi, jQuery)
