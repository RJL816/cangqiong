...
(function () {
    ...
    * */
    MyTool.prototype.message_soft = function (data) {
        ...
        // 判断是否已存在提示框，存在则先移除：
        var dom_message = dom_body.querySelector('.wtc_kztTool_message-soft-wrap');
        if (dom_message) {
            dom_message.parentElement.removeChild(dom_message);
            dom_message = null;
            clearTimeout(this.timer);
    ...
    * */
    MyTool.prototype.rightClickMenu = function (data) {
        ...
        var width = data.width ? data.width : "140px";
        var hideLeftIcon = data.hideLeftIcon;
        var hideRightIcon = data.hideRightIcon;
        html_func(data.data, true);

        function html_func(menuData, isRoot) {
            str += '\n                <div class="wtc_kztTool_rightClickMenu-group" style="position: absolute;display:' + (isRoot ? 'block' : 'none') + ';border-radius: 2px;box-shadow: rgba(0, 0, 0, 0.2) 2px 2px 3px 1px;background: #fff;box-sizing: border-box;width:'+ width +';">\n            ';
            ...
            for (var i = 0; i < menuData.length; i++) {
                ...
                for (var j = 0; j < arrGroup.length; j++) {
                    if (arrGroup[j].isHtml) {
                        ...
                        str += '\n                        <div class="wtc_kztTool_rightClickMenu-item" disabled="' + arrGroup[j].disabled + '" index="'
                            + arrGroup[j].index + '" style="position:relative;display:flex;padding:10px;color:'
                            + (arrGroup[j].disabled ? '#ccc' : '#333') + ';cursor:' + (arrGroup[j].disabled ? 'auto' : 'pointer')
                            + ';">\n                            <div class="wtc_kztTool_rightClickMenu-item-icon-left" style="flex:0 0 20px;'+(hideLeftIcon?'display:none;':'display:block;')+'"><i class="'
                            + arrGroup[j].iconLeft + '" style="font-size: 14px;"></i></div>\n                            <div class="wtc_kztTool_rightClickMenu-item-text" style="flex:1;margin-right:4px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;" title="' + arrGroup[j].value + '">' + arrGroup[j].value +
                            '</div>\n                            <div class="wtc_kztTool_rightClickMenu-item-icon-right" style="flex:0 0 20px;' +(hideRightIcon?'display:none;':'display:block;')+'"><i class="' + (arrGroup[j].children.length > 0 ? arrGroup[j].iconRight : '') + '" style="font-size: 14px;"></i></div>\n                        ';
    ...
    * */
        ...
        setTimeout(function () {
            ...
            if (data.target) {
                ...
                if (targetRect.left + dom_temp.scrollWidth > clientWidth && targetRect.bottom + dom_temp.scrollHeight > clientHeight) {
                    ...
                    if (data.showTriangle) {
                        var dom_triangle = document.querySelector('.wtc_kztTool_popover-triangle-bottom-right');
                    ...
                    if (data.showTriangle) {
                        var _dom_triangle = document.querySelector('.wtc_kztTool_popover-triangle-top-right');
                    ...
                    dom_temp.style.top = targetRect.top - dom_temp.scrollHeight + 'px';
                    if (data.showTriangle) {
                        var _dom_triangle2 = document.querySelector('.wtc_kztTool_popover-triangle-bottom-left');
                    ...
                    dom_temp.style.top = targetRect.bottom + 'px';
                    if (data.showTriangle) {
                        var _dom_triangle3 = document.querySelector('.wtc_kztTool_popover-triangle-top-left');
                ...
                if (data.positionX + dom_temp.scrollWidth > clientWidth && data.positionY + dom_temp.scrollHeight > clientHeight) {
                    ...
                    if (data.showTriangle) {
                        var _dom_triangle4 = document.querySelector('.wtc_kztTool_popover-triangle-bottom-right');
                    ...
                    if (data.showTriangle) {
                        var _dom_triangle5 = document.querySelector('.wtc_kztTool_popover-triangle-top-right');
                    ...
                    dom_temp.style.top = data.positionY - dom_temp.scrollHeight + 'px';
                    if (data.showTriangle) {
                        var _dom_triangle6 = document.querySelector('.wtc_kztTool_popover-triangle-bottom-left');
                    ...
                    dom_temp.style.top = data.positionY + 'px';
                    if (data.showTriangle) {
                        var _dom_triangle7 = document.querySelector('.wtc_kztTool_popover-triangle-top-left');
    ...
    MyTool.prototype.cancelPopover = function () {
        ...
        setTimeout(function () {
            if (!$this.popover_isEnter) {
                var dom_temp = document.querySelector('.wtc_kztTool_popover-wrap');
                dom_temp && dom_temp.parentElement && dom_temp.parentElement.removeChild(dom_temp);
                dom_temp = null;
            }
        }, 100);
    ...
    * */
        ...
        setTimeout(function () {
            ...
            if (data.target) {
                ...
                if (targetRect.left + dom_temp.scrollWidth > clientWidth && targetRect.bottom + dom_temp.scrollHeight > clientHeight) {
                    ...
                    dom_temp.style.top = targetRect.top - dom_temp.scrollHeight + 'px';
                    if (data.showTriangle) {
                        var dom_triangle = document.querySelector('.wtc_kztTool_tips-triangle-bottom-right');
                        if (dom_triangle) dom_triangle.style.display = 'block';
                    }
                } else if (targetRect.left + dom_temp.scrollWidth > clientWidth && targetRect.bottom + dom_temp.scrollHeight <= clientHeight) {
                    // 右上角
                    ...
                    dom_temp.style.top = targetRect.bottom + 'px';
                    if (data.showTriangle) {
                        var _dom_triangle8 = document.querySelector('.wtc_kztTool_tips-triangle-top-right');
                    ...
                    dom_temp.style.top = targetRect.top - dom_temp.scrollHeight + 'px';
                    if (data.showTriangle) {
                        var _dom_triangle9 = document.querySelector('.wtc_kztTool_tips-triangle-bottom-left');
                    ...
                    dom_temp.style.top = targetRect.bottom + 'px';
                    if (data.showTriangle) {
                        var _dom_triangle10 = document.querySelector('.wtc_kztTool_tips-triangle-top-left');
                ...
                if (data.positionX + dom_temp.scrollWidth > clientWidth && data.positionY + dom_temp.scrollHeight > clientHeight) {
                    ...
                    dom_temp.style.top = data.positionY - dom_temp.scrollHeight + 'px';
                    if (data.showTriangle) {
                        var _dom_triangle11 = document.querySelector('.wtc_kztTool_tips-triangle-bottom-right');
                    ...
                    dom_temp.style.top = data.positionY + 'px';
                    if (data.showTriangle) {
                        var _dom_triangle12 = document.querySelector('.wtc_kztTool_tips-triangle-top-right');
                    ...
                    dom_temp.style.top = data.positionY - dom_temp.scrollHeight + 'px';
                    if (data.showTriangle) {
                        var _dom_triangle13 = document.querySelector('.wtc_kztTool_tips-triangle-bottom-left');
                    ...
                    dom_temp.style.top = data.positionY + 'px';
                    if (data.showTriangle) {
                        var _dom_triangle14 = document.querySelector('.wtc_kztTool_tips-triangle-top-left');
    ...
    MyTool.prototype.cancelTips = function () {
        var dom_temp = document.querySelector('.wtc_kztTool_tips-wrap');
        dom_temp && dom_temp.parentElement && dom_temp.parentElement.removeChild(dom_temp);
        dom_temp = null;
    };
