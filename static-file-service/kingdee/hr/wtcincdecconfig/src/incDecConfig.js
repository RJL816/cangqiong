    ...
    class KdWtcIncDecConfig {
        ...
        * */
        legalInput() {
            ...
            // 判断是否匹配逻辑词的格式
            let patt = new RegExp(`\\s${this.lang_and}$|\\s${this.lang_or}$`, "g");
            let index = container.textContent.search(patt);
            // todo 1019
            if (index === -1) {
            ...
            if (this.arrCondition.includes(arrCondition[0])) {  // 删除
                ...
                for (let i = 0; i < arrCondition.length; i++) {
                    ...
                    if(arrTemp.length === 0 ) {
                        ...
                        this.args.conditionType === 1 ? arrTemp.push(this.lang_or, arrCondition[i]) : arrTemp.push(this.lang_and, arrCondition[i]);
                    }
                }
            }
