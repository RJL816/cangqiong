(function (KDApi, $) {
  ...
  var setHtml = function (model, props, isUpdate) {
    ...
    var template = '<div>'+
				   '<script id="'+editName+'" type="text/plain" style="width:100%;height:116px;"></script>' +
				   '</div>'
    var result = KDApi.template(template, {text:''})
    model.dom.innerHTML = result 
	KDApi.loadFile("formulaeditor.min.js", model, function(){
		model.dom.ue = UE.getEditor(editName,{allowDivTransToP: false},model);
  ...
  var updateHtml = function (model, props) {
    ...
    if ('getText'==operate){
		...
		var textLength = (addKeyText[1].length+3)*12;
		var newItem ='&nbsp;<input type="text" id="'+addKeyText[0]+'" class="invchangeformulatext" value="'+addKeyText[1]+'"  onfocus="this.blur()" style="width:'+textLength+'px;"/>&nbsp;'		
  ...
  var dealUpdateCommand = function(data,model){
		//1、先清空所有内容
		if(model.dom.ue.body.childNodes!=null || model.dom.ue.body.childNodes!=''){
  ...
  var buildUpdateData = function(model,data){
	...
	if (beginPosition>0){
		...
		var textLength = (tempList[1].length+3)*12;
		var buildStr = '&nbsp;<input type="text" id="'+tempList[0]+'" class="invchangeformulatext" value="'+tempList[1]+'"  onfocus="this.blur()"  style="width:'+textLength+'px;"/>&nbsp;'	
  ...
  // 注册自定义控件
  KDApi.register('invchangeformulatext', MyComponent)
})(window.KDApi, jQuery)
