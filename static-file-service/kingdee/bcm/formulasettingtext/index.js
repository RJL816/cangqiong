(function (KDApi, $) {
  ...
  var setHtml = function (model, props, isUpdate) {
	...
	KDApi.loadFile("formulaeditor.min.js", model, function(){
		model.dom.ue = UE_chk.getEditor(editName,{allowDivTransToP: false},model);
		//在dom结构完全准备好后才执行
  ...
  var updateHtml = function (model, props) {
    ...
    if ('getText'==operate){
		...
		var newItem ='&nbsp;<input type="text" id="'+addKeyText[0]+'" class="formulasettingtext" value="'+addKeyText[1]+'"  onfocus="this.blur()" style="width:'+textLength+'px;"/>&nbsp;'
		if(addKeyText.length == 3 && addKeyText[0].startsWith('dvbformulacb')){
			textLength = textLength + (addKeyText[2].length)*12;
			newItem ='&nbsp;<input type="text" id="'+addKeyText[0]+'" class="formulasettingtext" value="'+addKeyText[1]+','+addKeyText[2]+'"  onfocus="this.blur()" style="width:'+textLength+'px;"/>&nbsp;'
		}
		...
		var addKeyText = data.replace(/"/g,'').replace('{','').replace('}','').split(','); 
		var textLength = (addKeyText[1].length+1)*12;
		var newItem ='&nbsp;<input type="text" id="'+addKeyText[0]+'" class="formulasettingtext" value="'+addKeyText[1]+'"  onfocus="this.blur()" style="width:'+textLength+'px;background:none"/>&nbsp;'
		if(addKeyText.length == 3 && addKeyText[0].startsWith('dvbformulacb')){
			textLength = textLength + (addKeyText[2].length)*12;
			newItem ='&nbsp;<input type="text" id="'+addKeyText[0]+'" class="formulasettingtext" value="'+addKeyText[1]+','+addKeyText[2]+'"  onfocus="this.blur()" style="width:'+textLength+'px;background:none"/>&nbsp;'
		}
		model.dom.ue.execCommand('inserthtml',newItem,true);
		//绑定事件
		$("input",model.dom.ue.body).unbind('click').bind('click',function(){
  ...
  var buildUpdateData = function(model,data){
	...
	if (beginPosition>0){
		...
		if(tempList[0].startsWith('cvtformulacb'))	{
			buildStr = '<input type="text" id="'+tempList[0]+'" class="formulasettingtext" value="'+tempList[1]+'"  onfocus="this.blur()"  style="width:'+textLength+'px;background:none"/>'
		}else if(tempList.length == 3 && tempList[0].startsWith('dvbformulacb')){
			textLength = textLength + (tempList[2].length)*12;
			buildStr = '&nbsp;<input type="text" id="'+tempList[0]+'" class="formulasettingtext" value="'+tempList[1]+','+tempList[2]+'"  onfocus="this.blur()"  style="width:'+textLength+'px;"/>&nbsp;'
		}else{
			buildStr = '&nbsp;<input type="text" id="'+tempList[0]+'" class="formulasettingtext" value="'+tempList[1]+'"  onfocus="this.blur()"  style="width:'+textLength+'px;"/>&nbsp;'
		}
  ...
  // 注册自定义控件
  KDApi.register('formulasettingtext', MyComponent)
})(window.KDApi, jQuery)
