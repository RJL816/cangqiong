(function (KDApi, $) {
  ...
  var setHtml = function (model, props, isUpdate) {
	var editName = 'editor'+model.pageId.substring(18,model.pageId.length);
    var template = '<div>'+
				   '<script id="'+editName+'" type="text/plain" style="width:100%;height:102px;"></script>' +
				   '</div>'
    var result = KDApi.template(template, {text:''})
    model.dom.innerHTML = result 
	KDApi.loadFile("formulaeditor.min.js", model, function(){
		model.dom.ue = UE_biz.getEditor(editName,{allowDivTransToP: false},model);
		//在dom结构完全准备好后才执行
  ...
  var updateHtml = function (model, props) {
    ...
    if ('getText'==operate){
		...
		var content = data.replace(/"/g,'').replace('{','').replace('}','');
		var addKeyText = content.split(','); 
		var addContent = content.substr(content.indexOf(",") +1);
		var textLength = (addContent.length+1)*8;
		if(addKeyText[0].substring(addKeyText[0].length-2) == '_2')	{
			var newItem ='&nbsp;<input type="text" id="'+addKeyText[0]+'" class="bizconfigformulatext" value="'+addContent+'"  onfocus="this.blur()" style="width:'+textLength+'px;color:blue;border-color: blue;"/>&nbsp;'
		}else{
			var newItem ='&nbsp;<input type="text" id="'+addKeyText[0]+'" class="bizconfigformulatext" value="'+addContent+'"  onfocus="this.blur()" style="width:'+textLength+'px;"/>&nbsp;'
		}
		...
		var newItem ='&nbsp;<input type="text" id="'+addKeyText[0]+'" class="bizconfigformulatext" value="'+addKeyText[1]+'"  onfocus="this.blur()" style="width:'+textLength+'px;background:none"/>&nbsp;'
		model.dom.ue.execCommand('inserthtml',newItem,true);
		//绑定事件
		$("input",model.dom.ue.body).unbind('click').bind('click',function(){
			model.invoke('f(x)', $(this).attr('id'));	
  ...
  var dealUpdateCommand = function(data,model){
		...
		//3、解析内容，转换成显示的格式
		//var con = data.replace(/"/g,'').replace('{','').replace('}','');
		//var start = con.indexOf(',');
		//var end = con.indexOf('}');
		//var dataList = con.substr(start + 1).split(';');
		...
		for(var k =0;k<dataList.length;k++){
			...
			if (k != dataList.length -1) {
				model.dom.ue.execCommand('inserthtml', ';</br>',true);
			}
		}
  ...
  var buildUpdateData = function(model,data){
	...
	if (beginPosition>0){
		...
		var con = data.replace(/"/g,'');
		var tempData = con.substring(1,con.indexOf('}'));
		var tempKey = tempData.substring(0,tempData.indexOf(','));
		var tempValue = tempData.substring(tempData.indexOf(',')+1,tempData.length + 1);
		var textLength = (tempValue.length+1)*8;
		var buildStr;
		if(tempKey.substring(tempKey.length-2) == '_2')	{
			buildStr = '<input type="text" id="'+tempKey+'" class="bizconfigformulatext" value="'+tempValue+'"  onfocus="this.blur()"  style="width:'+textLength+'px;color:blue;border-color: blue;"/>'
		}else{
			buildStr = '&nbsp;<input type="text" id="'+tempKey+'" class="bizconfigformulatext" value="'+tempValue+'"  onfocus="this.blur()"  style="width:'+textLength+'px;"/>&nbsp;'
		}
  ...
  // 注册自定义控件
  KDApi.register('bizconfigformulatext', MyComponent)
})(window.KDApi, jQuery)
