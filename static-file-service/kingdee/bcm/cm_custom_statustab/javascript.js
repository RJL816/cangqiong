...
function updateTabs(statusTabApId,tabs,focusTabId){
	...
	for (let tab of tabs){
		...
		if (!isNull(tabItemDiv)){
			updateTabItemDivInfo(tabItemDiv,tab.tabName,tab.tabStatusName,tab.tabStatusBackColor,tab.showClose,tab.iconClass);
...
function focusTab(tabItemDiv){	
    ...
    // 上边框颜色
    tabStyle.borderBottom = tabAp.attributes.tabFocusBorderTopStyle && tabAp.attributes.tabFocusBorderTopStyle.value || "";
    // 背景色
    ...
    for (var i = 0; i < tabs.length; i++) {
        ...
        tabs[i].style.borderBottom = tabAp.attributes.tabUnFocusBorderTopStyle && tabAp.attributes.tabUnFocusBorderTopStyle.value || "";
...
function updateTabItemDivInfo(tabItemDiv,tabName,statusName,tabStatusBackColor,showClose,iconClass){
	...
	tabItemDiv.setAttribute("iconClass",iconClass);
	var children = tabItemDiv.children;
	if (!isNull(children)){
		for(var i=0;i<children.length;i++){
			...
			if (classValue == "kingdee-eb-statustab-tabitemname"){
				...
				children[i].style.bac = tabStatusBackColor;
				children[i].style.padding = isBlank(statusName)? "0 0 0 0" : "0 6px 0 6px";
			} else if (classValue == "kingdee-eb-statustab-tabitemicon"){
				children[i].children[0].src = iconClass;
			}
		}
	}
...
function getTabInnerHTML(statusTabApId,tabId,tabName,statusName,showClose,tabStatusBackColor,tabNameMaxWidth,tabStatusMaxWidth,iconClass,iconColor){
	...
	// innerHTML += '<div class="kingdee-eb-statustab-tabitemstatus" title="'+ statusName +'" style="background-color:'+ tabStatusBackColor + ';color:'+ tabStatusBackColor + ';border-color:' + tabStatusBackColor +';max-width:'+tabStatusMaxWidth+';padding:'+ statusNamePadding +';"><span class="kingdee-eb-statustab-span">'+ statusName +'</span></div>';
	// innerHTML += '<div class="kingdee-eb-statustab-tabitemstatus" title="'+ statusName +'" style="background-color:'+ tabStatusBackColor + ';color:'+ tabStatusBackColor + ';border-color:' + tabStatusBackColor +';max-width:'+tabStatusMaxWidth+';padding:'+ statusNamePadding +';"></div>';
	// 状态
	innerHTML += '<div class="kingdee-eb-statustab-tabitemicon"><img src="' + iconClass +'" class"></div>';
    // 页签名称
