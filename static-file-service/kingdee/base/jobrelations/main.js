...
(function (KDApi) {
  ...
  var setHtml = function (model, primaryProps) {
    KDApi.loadFile(['./css/index.css', './css/style.less'], model, () => {
      class Root extends React.Component {
        ...
        componentDidMount () {
          ...
          this.updateSub = eventBus.sub(model, 'update', (updateProps) => {
            this.setState({ customProps: Object.assign({}, updateProps) }, () => {
              this.forceUpdate()
            })
          })
        }
        ...
        render () {
          ...
          const data = customProps.data ? [].concat(customProps.data) : []
          return (
            <OrgChart model={model} data={data} />
          )
        }
      }
  ...
  // 注册自定义组件
  KDApi.register('jobrelations', MyComponent, {
    isMulLang: true
  })
})(window.KDApi)
