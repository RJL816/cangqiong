...
var SeeksGraphMath = {
  ...
  getCirclePoint4MultiLine: function (x1, y1, x2, y2, n1w, n1h, n2w, n2h, isReserve, allSize, indexOfAll) {
    // CommonUtils.log(indexOfAll, 'of', allSize, isReserve)
    if (isReserve) {
      indexOfAll = allSize - indexOfAll - 1;
      // CommonUtils.log(indexOfAll, 'of', allSize, '|indexOfAll changed!')
    }
    var to_x = x2 + n2w / 2;
    var to_y = y2 + n2h / 2;
    var from_x = x1 + n1w / 2;
  ...
  analysisNodes: function (willLayoutNodes, thisLevelNodes, thisDeep, analyticResult, config) {
    ...
    // var __prev_node
    thisLevelNodes.forEach(thisNode => {
      // CommonUtils.log('Build node::', thisNode.name, thisNode.targetNodes.length)
        ...
        thisNode.targetNodes.forEach(thisTarget => {
          ...
          if (!thisTarget.lot.eached) {
            if (SeeksGraphMath.isAllowShowNode(thisTarget)) {
              ...
              thisNode.lot.childs.push(thisTarget);
              // CommonUtils.log('hide node:', thisTarget.name, 'from:', thisNode.text)
            }
          }
        });
      }
    ...
    // CommonUtils.log(thisDeep, 'next level nodes:', newLevelNodes.length)
    if (newLevelNodes.length > 0) {
      // CommonUtils.log('thisLevelNodes.length:', thisLevelNodes, thisLevelNodes.length)
  ...
  analysisNodes4Didirectional: function (willLayoutNodes, thisLevelNodes, thisDeep, analyticResult, levelDirect) {
    ...
    thisLevelNodes.forEach(thisNode => {
      var __thisNode_child_size = 0;
      if (levelDirect === 0) {
        // CommonUtils.log('Build node::from::', thisNode.name, thisNode.targetNodes.length)
        let __thisTargetIndex = 0;
        thisNode.targetNodes.forEach(thisTarget => {
          if (!thisTarget.lot) thisTarget.lot = { eached: false };
          if (!thisTarget.lot.eached) {
            if (SeeksGraphMath.isAllowShowNode(thisTarget)) {
              ...
              thisNode.lot.childs.push(thisTarget);
              // CommonUtils.log('hide node:', thisTarget.text, 'from:', thisNode.text)
            }
          } else {
              // CommonUtils.log('solved node:', thisTarget.text, 'from:', thisNode.text)
            }
        });
      } else if (levelDirect === -1) {
        // CommonUtils.log('Build node::from::', thisNode.name, thisNode.targetFrom.length)
        let __thisTargetIndex = 0;
        ...
        // CommonUtils.log('Build node::to::', thisNode.name, thisNode.targetTo.length)
        let __thisTargetIndex = 0;
  ...
  analysisDataTree: function (thisLevelNodes, thisDeep, levelDirect) {
    ...
    thisLevelNodes.forEach(thisNode => {
      // CommonUtils.log('Place node aaaaaa:', levelDirect, thisNode.text, (thisNode.lot.level < 0 ? -1 : 1))
  ...
  isAllowShowNode: function (thisNode) {
    ...
    // if (derict !== false && _r === false) CommonUtils.log('hide node by:', thisNode.isShow !== false, thisNode.isHide !== true)
    return _r;
  }
};
...
function SeeksBidirectionalTreeLayouter(layoutSetting, graphSetting) {
  ...
  CommonUtils.log('new SeeksBidirectionalTreeLayouter:', this.config);
  ...
  this.refresh = function () {
    CommonUtils.log('SeeksBidirectionalTreeLayouter:refresh:nodes:', this.__origin_nodes.length);
  ...
  this.analysisNodes4Didirectional = function (willLayoutNodes, thisLevelNodes, thisDeep, analyticResult, levelDirect) {
    ...
    thisLevelNodes.forEach(thisNode => {
      var __thisNode_child_size = 0;
      // CommonUtils.log('Build node::', thisNode.text, thisNode.targetNodes.length)
      if (levelDirect === -1) {
        // CommonUtils.log('Build node::from::', thisNode.text, thisNode.targetFrom.length)
        let __thisTargetIndex = 0;
        thisNode.targetFrom.forEach(thisTarget => {
          if (!thisTarget.lot) thisTarget.lot = { eached: false };
          if (!thisTarget.lot.eached) {
        ...
        // CommonUtils.log('Build node::to::', thisNode.text, thisNode.targetTo.length)
        let __thisTargetIndex = 0;
        thisNode.targetTo.forEach(thisTarget => {
          if (!thisTarget.lot) thisTarget.lot = { eached: false };
          if (!thisTarget.lot.eached) {
  ...
  this.placeNodes = function (allNodes, rootNode) {
    CommonUtils.log('SeeksBidirectionalTreeLayouter:placeNodes');
    if (!rootNode) {
      console.error('root is null');
      return;
    } else {
      CommonUtils.log('layout by root:', rootNode);
    }
  ...
  this.placeNodesPosition = function (rootNode, allNodes, analyticResult) {
    ...
    // CommonUtils.log('analysisNodes:', analyticResult, allNodes)
    ...
    // CommonUtils.log('#############Seeks graph viewSize:Tree layout:', this.graphSetting.viewSize.width, this.graphSetting.viewSize.height)
    // CommonUtils.log('[layout canvasOffset]', __mapHeight, this.graphSetting, this.graphSetting.canvasSize, this.config)
    // CommonUtils.log('[Layout:AnalyticResult]', analyticResult)
    if (rootNode.fixed !== true) {
      ...
      CommonUtils.log('设置根节点位置:', rootNode.text, rootNode.x, rootNode.y, this.graphSetting.canvasSize.width, this.graphSetting.canvasSize.height, this.graphSetting.canvasOffset.x, this.graphSetting.canvasOffset.y);
      rootNode.x = rootNode.lot.x + __offsetX;
      rootNode.y = rootNode.lot.y + __offsetY;
    } else {
      CommonUtils.log('固定位置的rootNode:', rootNode.text, rootNode.x, rootNode.y);
    ...
    allNodes.forEach(thisNode => {
      ...
      // CommonUtils.log(thisNode.text, thisNode.offset_x, thisNode.offset_y)
  ...
  this.placeRelativePosition = function (rootNode, analyticResult, dynamicSizeConfig) {
    if (this.config.from === 'left' || this.config.from === 'right') {
      ...
      this.allNodes.forEach(thisNode => {
        ...
        // CommonUtils.log('Place node:', thisNode.text, thisNode)
        // CommonUtils.log('Place node lot:', thisNode.lot.subling.level, thisNode.lot.index_of_level, 'of', thisNode.lot.subling.all_size, thisNode.lot.subling.all_strength)
      ...
      this.allNodes.forEach(thisNode => {
        ...
        if (this.config.from === 'bottom') {
          ...
          // CommonUtils.log('Place node xxxx:', rootNode.lot.y, thisNode.lot.subling.level, __per_height)
          thisNode.lot.y = rootNode.lot.y + this.getLevelDistance(thisNode, thisNode.lot.subling.level, __per_height);
        }
      });
      this.allNodes.forEach(thisNode => {
        if (thisNode.fixed === true) return;
        if (thisNode.lot.level !== 0) {
          // CommonUtils.log('Place node xxxx:', thisNode.lot.strengthWithChilds_from, thisNode.lot.strengthWithChilds, __per_width)
...
function SeeksCenterLayouter(layoutSetting, graphSetting) {
  ...
  this.__origin_nodes = [];
  this.refresh = function () {
    if (window.SeeksGraphDebug) CommonUtils.log('SeeksCenterLayouter:refresh');
    this.placeNodes(this.__origin_nodes, this.rootNode);
  };
  this.placeNodes = function (allNodes, rootNode) {
    if (window.SeeksGraphDebug) CommonUtils.log('SeeksCenterLayouter:placeNodes');
    if (!rootNode) {
      CommonUtils.log('root is null:', rootNode);
      return;
    } else {
      if (window.SeeksGraphDebug) CommonUtils.log('layout by root:', rootNode);
    }
    this.__origin_nodes = allNodes;
    this.rootNode = rootNode;
    allNodes.forEach(thisNode => {
    ...
    // CommonUtils.log('analysisNodes:', analyticResult)
    // if (this.graphSetting.heightByContent) {
    //   CommonUtils.log('根据内容调整高度')
    //   var __suitableHeight = analyticResult.max_deep * 2 * 300 + 500
    //   this.graphSetting.viewSize.height = __suitableHeight
    // }
    if (window.SeeksGraphDebug) CommonUtils.log('调整画布大小');
    ...
    // CommonUtils.log('per:', __per_width, __per_height)
    ...
    //     CommonUtils.log('level2 parents:', thisNode.name, thisNode.lot.childs_size, { strength_plus: thisNode.lot.strength_plus, __thisNodeLength, strength: thisNode.lot.childs_size, __level2_current_length })
    //   }
    // })
    ...
    // }
    // CommonUtils.log('[layout canvasOffset]', this.graphSetting.viewSize, this.graphSetting.canvasSize)
    ...
    //   }
    // })
    CommonUtils.log('Start Auto Layout.....');
    // this.autoLayout(true)
    // CommonUtils.log('layout from root:', analyticResult.max_deep, analyticResult.max_length)
  ...
  this.placeRelativePosition = function (rootNode, analyticResult) {
    ...
    if (window.SeeksGraphDebug) CommonUtils.log('analyticResult:', analyticResult, __leve1_min_r, this.config.distance_coefficient);
  ...
  this.autoLayout = function (forceLayout) {
    ...
    CommonUtils.log('this.layoutTimes:', this.layoutTimes);
    ...
    if (__by_node) {
      for (const i in this.__origin_nodes) {
        ...
        // if (__node1.text === '宣洁')CommonUtils.log('宣洁:', __node1.x, __node1.y)
  ...
  this.addGravityByNode = function (node1, node2) {
    ...
    // CommonUtils.log({ Kf, _buff_x, _buff_y, zero_length })
  ...
  this.addFtoNode = function (node, x, y) {
    // CommonUtils.log('Add F:', node.text, type, parseInt(x), parseInt(y))
    if (isNaN(x) || isNaN(y)) {
      return;
    }
    ...
    if (y < -50) y = -50;
    // if (isNaN(node.Fx)) {
    //   if (node.text === '宣洁')CommonUtils.log('宣洁!!!NaN B buff x:', x, node.lot.strength, node)
    // }
    node.Fx += x;
    node.Fy += y;
    // if (isNaN(node.Fx)) {
    //   if (node.text === '宣洁')CommonUtils.log('宣洁!!!NaN A buff x:', x, node.lot.strength, node)
    // }
  };
  this.applyToNodePosition = function (node) {
    ...
    //   return
    // }
    // CommonUtils.log('F add:', node.name, node.Fx, node.Fy)
    const __buff_x = parseInt(node.Fx);
    const __buff_y = parseInt(node.Fy);
    // CommonUtils.log('F add:2:', node.name, __buff_x, __buff_y)
    node.x = node.x + __buff_x;
    node.y = node.y + __buff_y;
    // if (isNaN(node.x)) {
    //   if (node.text === '宣洁')CommonUtils.log('!!!NaN x:', node.text, __buff_x, node.Fx, node)
    // }
    // node.name = __buff_x + ',' + __buff_y
    // if (node.id === '8') {
    //   CommonUtils.log(node.id, __buff_x, __buff_y)
    // // CommonUtils.log(node.x, node.y)
    // }
    node.Fx = 0;
    node.Fy = 0;
  };
...
function SeeksCircleLayouter_SeeksCenterLayouter(layoutSetting, setting) {
  ...
  this.checkMaxDeepAndLength = function (thisLevelNodes, thisDeep) {
    ...
    thisLevelNodes.forEach(thisNode => {
      ...
      if (thisNode.targetNodes) {
        thisNode.targetNodes.forEach(thisTarget => {
          CommonUtils.log('child node::', thisTarget.type, thisTarget.lot.eached);
          if (!thisTarget.lot) thisTarget.lot = { eached: false };
    ...
    // CommonUtils.log('next level nodes:', newLevelNodes.length)
    if (newLevelNodes.length > 0) {
      // CommonUtils.log('thisLevelNodes.length:', thisLevelNodes, thisLevelNodes.length)
  ...
  this.placeNodes = function (allNodes, rootNode) {
    ...
    this.allNodes = [];
    CommonUtils.log('max before:', this.__max_deep, this.__max_length);
    this.checkMaxDeepAndLength([this.rootNode], 0);
    CommonUtils.log('max after:', this.__max_deep, this.__max_length);
    ...
    this.allNodes.forEach((thisNode, _index) => {
      ...
      thisNode.x = _point.x;
      thisNode.y = _point.y;
      // CommonUtils.log('Place node:', thisNode.name, thisNode.x, thisNode.y)
    ...
    CommonUtils.log('Start Auto Layout.....');
    // this.autoLayout(true)
    // CommonUtils.log('layout from root:', this.__max_deep, this.__max_length)
  ...
  this.adjustLevel2Y = function (__mapHeight) {
    for (let i = 0; i < this.allNodes.length; i++) {
      ...
      if (thisNode.lot.subling.level === 1 && thisNode.lot.childs_size === 0) {
        ...
        CommonUtils.log(__per_height, __mapHeight, thisNode.lot.subling.all_size, thisNode.lot.subling.all_strength, thisNode.lot.strength);
        for (let j = 0; j < this.allNodes.length; j++) {
          ...
          if (thisLevel2Node.lot.subling.level === 1 && thisLevel2Node !== thisNode) {
            ...
            if (__y_diff < __per_height - 2) {
              CommonUtils.log('__y_diff', thisNode.name, thisLevel2Node.name, __y_diff);
...
function SeeksAutoLayouter(layoutSetting, graphSetting) {
  ...
  this.placeNodes = function (allNodes, rootNode) {
    ...
    //   this.rootNode.lot.y = 400
    // }
    if (window.SeeksGraphDebug) CommonUtils.log('[layout canvasOffset]', this.graphSetting.viewSize, this.graphSetting.canvasSize);
    ...
    //   }
    // })
    if (window.SeeksGraphDebug) CommonUtils.log('Start Auto Layout.....');
    this.autoLayout(true);
    // CommonUtils.log('layout from root:', analyticResult.max_deep, analyticResult.max_length)
    // rootNode.x = (this.graphSetting.canvasSize.width - this.graphSetting.nodeSize.width) / 2
    // rootNode.y = (this.graphSetting.canvasSize.height - this.graphSetting.nodeSize.height) / 2
  ...
  this.autoLayout = function (forceLayout) {
    ...
    if (window.SeeksGraphDebug) CommonUtils.log('this.layoutTimes:', this.layoutTimes);
    if (this.layoutTimes > 300) {
      this.graphSetting.autoLayouting = false;
      return;
    }
...
var SeeksRGUtils = {
  startDrag(e, positionModel, ondraged) {
    __ondraged = ondraged;
    // CommonUtils.log('startDrag:', __tmp_basePosition, e.clientX, e.clientY)
  ...
  onNodeMove(e) {
    // CommonUtils.log('move', __tmp_basePosition, e.clientX, e.clientY)
    __tmp_positionModel.x = e.clientX + __tmp_basePosition.x;
    __tmp_positionModel.y = e.clientY + __tmp_basePosition.y;
  },
  onNodeDragend() {
    // CommonUtils.log('onNodeDragend', __tmp_positionModel.x - __start_info.x, __tmp_positionModel.y - __start_info.y)
...
const SeeksStoreManager = {
  createDefaultConfig(userGraphSetting) {
    ...
    if (_debug) CommonUtils.log('user instance graphSetting:', userGraphSetting);
    if (window) {
    ...
    if (userGraphSetting) {
      Object.keys(userGraphSetting).forEach(key => {
        ...
        if (typeof _thisUserValue === 'object') {
          if (window.SeeksGraphDebug) CommonUtils.log('user setting object:', key, _thisUserValue);
          ...
          if (_objectValue) {
            if (_objectValue && !Array.isArray(_objectValue) && _thisUserValue) {
              Object.keys(_objectValue).forEach(l2Key => {
                if (window.SeeksGraphDebug) CommonUtils.log('   user setting:', key + '.' + l2Key, _thisUserValue[l2Key]);
                _objectValue[l2Key] = _thisUserValue[l2Key];
              });
            } else if (Array.isArray(_objectValue)) {
              if (window.SeeksGraphDebug) CommonUtils.log('   user setting array:', key, 'size:', _thisUserValue.length);
              var _new_arr = [];
              _thisUserValue.forEach(thisItem => {
                if (window.SeeksGraphDebug) CommonUtils.log('   user setting array:', key, 'push:', thisItem);
              ...
              // if (window.SeeksGraphDebug) CommonUtils.log('   user setting array:', key, 'copy size:', _new_arr.length)
            } else {
              if (window.SeeksGraphDebug) CommonUtils.log('user setting value:', key);
              _graphSetting[key] = _thisUserValue;
            }
          } else {
            CommonUtils.log('ignore option:', key);
          }
        } else {
          if (window.SeeksGraphDebug) CommonUtils.log('user setting:', key, _thisUserValue);
          _graphSetting[key] = _thisUserValue;
        }
      });
    }
  ...
  createNewStore(userGraphSetting) {
    if (window.SeeksGraphDebug) CommonUtils.log('Create new GraphSetting:');
...
function SeeksRGStore(_graphSetting) {
  ...
  this.resetViewSize = function () {
    ...
    // CommonUtils.log('resetViewSize:', state.graphSetting.viewSize.width, state.graphSetting.canvasSize.width, state.graphSetting.canvasZoom / 100, state.graphSetting.canvasSize.width * (state.graphSetting.canvasZoom / 100), state.graphSetting.canvasOffset.x)
  };
  ...
  CommonUtils.log('relation-graph instance full option:', this.getOptions());
}
...
/* harmony default export */ var SeeksRGNode = ({
  ...
  watch: {
    ...
    //   CommonUtils.log('nodeProps.isShow:', v)
    //   if (v === true) {
    //     this.$nextTick(() => {
    ...
    //       CommonUtils.log('node 挂载 el size:', this.$refs.seeksRGNode.offsetWidth, this.$refs.seeksRGNode.offsetHeight)
    //     })
    //   }
    // }
  },
  ...
  mounted() {
    ...
    // CommonUtils.log('node show:', this.nodeProps.text, this.$parent.$slots.node)
  },
  beforeDestroy() {
  ...
  methods: {
    refreshNodeProperties() {
      ...
      // CommonUtils.log('node 挂载 el:', this.nodeProps.text, this.nodeProps.el.offsetWidth, this.nodeProps.el.offsetHeight)
      // this.$nextTick(() => {
      //   this.nodeProps.el.offsetWidth = this.$refs.seeksRGNode.offsetWidth
      //   this.nodeProps.el.offsetHeight = this.$refs.seeksRGNode.offsetHeight
      //   CommonUtils.log('node 挂载 el size:', this.$refs.seeksRGNode.offsetWidth, this.$refs.seeksRGNode.offsetHeight)
      ...
      //   CommonUtils.log('resize node name:', this.name)
      // }
    ...
    onNodeDraged(x, y) {
      ...
      if (Math.abs(x) + Math.abs(y) > 6) {
        setTimeout(function () {
          if (window.SeeksGraphDebug) CommonUtils.log('delay end dragging', this.dragging);
          this.dragging = false;
        }.bind(this), 100);
      } else {
        this.dragging = false;
    ...
    // beforeEnter(el) {
    //   CommonUtils.log('beforeEnter')
    //   el.style.opacity = 0
    //   el.style.transformOrigin = 'left'
    // },
    // enter(el, done) {
    //   CommonUtils.log('enter')
    //   Velocity(el, { opacity: 1, fontSize: '1.4em' }, { duration: 300 })
    //   Velocity(el, { fontSize: '1em' }, { complete: done })
    // },
    // leave(el, done) {
    //   CommonUtils.log('leave')
    ...
    getLightColor(col) {
      ...
      if (col[0] === '#') {
        ...
        if (window.SeeksGraphDebug) CommonUtils.log('getLightColor1:', col, ':', _rgb_arr.join(','));
        col = 'rgb(' + _rgb_arr.join(',') + ')';
      }
      var _st = col.substring(col.indexOf('(') + 1);
      _st = _st.substring(0, _st.indexOf(')'));
      var _rgb_string = _st.split(',');
      // CommonUtils.log('getLightColor444:', _st, ':', _rgb_string.join(','))
      if (_rgb_string.length >= 3) {
        ...
        if (window.SeeksGraphDebug) CommonUtils.log('getLightColor2:', col, ':', _rgb_number.join(','));
...
/* harmony default export */ var SeeksRGLink = ({
  ...
  methods: {
    ...
    createLinePath(from, to, ri, relationData) {
      // CommonUtils.log('redrawLine:', this.lineProps.fromNode.id, this.lineProps.toNode.id, ri)
      // CommonUtils.log('_point:', _point)
      if (!ri) ri = 0;
      ...
      if (isNaN(f_W) || isNaN(f_H)) {
        // CommonUtils.log('error from node size:', f_W, f_H)
        relationData.textPositon.x = 50;
        relationData.textPositon.y = 50;
        relationData.textPositon.rotate = 0;
        return 'M 0 0 L 100 100';
      ...
      if (isNaN(t_W) || isNaN(t_H)) {
        // CommonUtils.log('error to node size:', f_W, f_H)
        relationData.textPositon.x = 50;
        relationData.textPositon.y = 50;
        relationData.textPositon.rotate = 0;
        return 'M 0 0 L 100 100';
      ...
      if (__lineShape === 4) {
        ...
        if (__lineDirection === 'v') {
          ...
          // CommonUtils.log('start:', __start, __end, __buff_x, __buff_y)
          __path = 'M' + fx + ',' + fy + ' c' + __buff_type * 30 + ',' + 0 + ' ' + __buff_type * -10 + ',' + __buff_y * distanceRate + ' ' + __buff_x + ',' + __buff_y;
        }
        ...
        if (isNaN(relationData.textPositon.rotate)) {
          relationData.textPositon.rotate = 0;
          CommonUtils.log('NaN rotate:', relationData);
        }
    ...
    isAllowShowNode: function (thisNode) {
      const _r = thisNode.isShow !== false && thisNode.isHide !== true && (!thisNode.lot.parent || this.isAllowShowNode(thisNode.lot.parent, false) === true);
      // if (derict !== false && _r === false) CommonUtils.log('be hide node:', thisNode.text)
      return _r;
    },
    flash() {}
  }
...
// import { mapState } from 'vuex'
// var _parent = this.$parent
// CommonUtils.log('GraphSettingPanel.vue:', _parent)
/* harmony default export */ var GraphSettingPanel = ({
  ...
  //     CommonUtils.log('change layout:', newV, oldV)
  ...
  methods: {
    ...
    switchLayout() {
      if (window.SeeksGraphDebug) CommonUtils.log('change layout:', this.currentLayoutName);
...
/* harmony default export */ var GraphMiniView = ({
  ...
  methods: {
    getPositionData() {
      ...
      // CommonUtils.log('Mini View style:', _view_center_x, _canvas_center_x)
      var style = {
        width: _width + 'px',
        height: _height + 'px',
...
/* harmony default export */ var GraphMiniToolBar = ({
  ...
  methods: {
    ...
    switchLayout(layoutConfig) {
      if (window.SeeksGraphDebug) CommonUtils.log('change layout:', layoutConfig);
      core4vue_SeeksRGLayouters.switchLayout(layoutConfig, this.graphSetting);
      this.refresh();
    },
    toggleAutoLayout() {
      // CommonUtils.log('this.graphSetting.autoLayouting', this.graphSetting.autoLayouting)
...
var GraphMiniToolBar_render = function () {
  ...
  return _c("div", {
    ...
    style: {
      //经测试,如果不加display控制,切换页签时工具栏会左右跳动
      "display": _vm.graphSetting.viewELSize.width >= 10 ? 'block': 'none',
      //设置工具栏位于左上角,否则x轴滚动条可能会隐藏该工具栏
      "margin": 0,
    }
  }, [_c("div", {
  ...
  }, [_c("svg", { staticClass: "rg-icon", attrs: { "aria-hidden": "true" } }, [_c("use", { attrs: { "xlink:href": "#icon-resize-" } })]), _vm._v(" "), _c("span", { staticClass: "c-mb-text" }, [_vm._v(_vm._s(_vm.graphSetting.fullscreen ? "全屏" : "全屏"))])]), _vm._v(" "), _vm.graphSetting.allowShowZoomMenu ? _c("div", {
    staticClass: "c-mb-button",
    on: {
      click: function ($event) {
    ...
    staticClass: "c-mb-child-panel",
    staticStyle: { width: "256px", "margin-left": "41px" }
  }, [_c("div", {
    staticClass: "c-mb-button c-mb-button-c",
    class: {
    ...
    staticClass: "c-mb-child-panel",
    staticStyle: { width: "206px", "margin-left": "41px" }
  }, [_c("div", {
    staticClass: "c-mb-button c-mb-button-c",
    class: {
...
/* harmony default export */ var GraphBottomPanel = ({
  ...
  mounted() {
    this.$nextTick(() => {
      ...
      if (window.SeeksGraphDebug) CommonUtils.log('SeeksGraph bootomPanel height:', this.height);
    });
  },
  methods: {}
});
...
/* harmony default export */ var selectortype_script_index_0_src = ({
  ...
  data() {
    var wheelEvent = {};
    // CommonUtils.log('this.options.disableZoom:', this.options.disableZoom)
    ...
    //     'mousewheel':this.mouseListener
    //   }
    //   CommonUtils.log('wheelEvent:', wheelEvent)
    // }
    return {
      wheelEvent,
      version: '1.1.0',
  ...
  methods: {
    ...
    //   CommonUtils.log('重新计算:getCanvasSizeAndPosition')
    //   return {
    //     'width': this.graphSetting.canvasSize.width + 'px',
    //     'height': this.graphSetting.canvasSize.height + 'px',
    //     'margin-left': (this.graphSetting.canvasOffset.x) + 'px',
    ...
    init() {
      ...
      // CommonUtils.log('#############Seeks graph viewSize:', this.graphSetting.viewSize.width, this.graphSetting.viewSize.height)
      this.cycleTask();
      this.resetViewSize();
      this.refreshNVAnalysisInfo();
      this.syncToolsPosition();
    ...
    cycleTask() {
      if (this.alive) {
        ...
        if (_box.width !== this.graphSetting.viewSize.width || _box.height !== this.graphSetting.viewSize.height) {
          if (window.SeeksGraphDebug) CommonUtils.log('relation-graph:view-size-changed:', [this.graphSetting.viewSize.width, this.graphSetting.viewSize.height], [_box.width, _box.height]);
          ...
          if (this.viewSizeIsInited && this.graphSetting.moveToCenterWhenResize) {
            if (window.SeeksGraphDebug) CommonUtils.log('relation-graph:move to center:', [this.graphSetting.viewSize.width, this.graphSetting.viewSize.height], [_box.width, _box.height]);
            this.$nextTick(() => {
              // this.focusRootNode()
              var _min_x = 9999999;
              var _min_y = 9999999;
              ...
              // if (window.SeeksGraphDebug) CommonUtils.log('_min_x, _min_y, _stuff_width, _stuff_height:', _min_x, _min_y, _stuff_width, _stuff_height)
              // var _final_x = -_min_x
              // var _final_y = -_min_y
    ...
    setOptions(options, callback) {
      ...
      if (this.graphSetting.layouts && this.graphSetting.layouts.length > 0) {
        ...
        if (window.SeeksGraphDebug) CommonUtils.log('创建默认布局器：', this.graphSetting.layoutName);
        ...
        CommonUtils.log('你需要设置layouts来指定当前图谱可以使用的布局器！');
      }
      this.doLayout();
      callback(this);
    },
    ...
    mouseListenerEmpty() {
      CommonUtils.log('mouseListenerEmpty');
    },
    mouseListener(e) {
      ...
      var userZoomCenter = {
        x: e.clientX,
        y: e.clientY
        // CommonUtils.log('---- center:', userZoomCenter.x, userZoomCenter.y)
      };var _isMac = /macintosh|mac os x/i.test(navigator.userAgent);
      var _deltaY = e.deltaY;
      if (_deltaY === undefined) {
        _deltaY = e.wheelDelta;
      }
      // CommonUtils.log('mouseListenerEmpty:', _isMac, e.deltaY, e.wheelDelta, e.which, e.detail)
    ...
    getPositionOfCanvas(e) {
      ...
      if (window.SeeksGraphDebug) CommonUtils.log('[F]', userZoomCenter.x, userZoomCenter.y);
      var currentNode = e.target.parentNode;
      for (var i = 0; i < 8; i++) {
        if (i > 6) {
          if (window.SeeksGraphDebug) CommonUtils.log('getPositionOfCanvas error', e);
        }
        if (currentNode.classList.contains('rel-map-canvas')) {
          if (window.SeeksGraphDebug) CommonUtils.log('[S]', currentNode.tagName + '.' + currentNode.className);
          break;
        } else {
          userZoomCenter.x += currentNode.offsetLeft || 0;
          userZoomCenter.y += currentNode.offsetTop || 0;
          if (window.SeeksGraphDebug) CommonUtils.log('[' + i + ']', currentNode.tagName + '.' + currentNode.className, ':', currentNode.offsetLeft, currentNode.offsetTop);
    ...
    zoom(buff, userZoomCenter) {
      ...
      // CommonUtils.log('zoomCenter:', zoomCenter.x, zoomCenter.y)
      ...
      // CommonUtils.log('offset buff:', _buff_x.toFixed(0), _buff_y.toFixed(0))
    ...
    showZoomCenter(userZoomCenter, zoomBuff) {
      ...
      // CommonUtils.log('this.currentZoomSet:', this.currentZoomSet)
      // CommonUtils.log('this.currentZoomSet:', this.newZoomSet)
    ...
    refreshNVAnalysisInfo() {
      ...
      // CommonUtils.log('reanalysis NV info...')
      var result = {
        NMCanvasCenter: { x: 0, y: 0 }
      };
    ...
    syncToolsPosition() {
      if (window.SeeksGraphDebug) CommonUtils.log('on scroll...');
      if (!this.$refs.seeksRelationGraph) return;
      const windowHeight = this.getWindowHeight();
      var _box_info = this.$refs.seeksRelationGraph.getBoundingClientRect();
      if (window.SeeksGraphDebug) CommonUtils.log('syncToolsPosition...');
      // CommonUtils.log('change layout:', __top, this.$refs.seeksRelationGraph.offsetHeight)
      // CommonUtils.log(_box_info.top, this.viewOffset.positionTop, (this.viewOffset.windowHeight - this.viewOffset.top))
    ...
    loadLinks(_links) {
      _links.forEach(thisLinkJson => {
        ...
        // CommonUtils.log('[add link]', __from.text, __to.text, __from.seeks_id, __to.seeks_id, thisLink)
        ...
        // CommonUtils.log('new Line Color:', thisLine.color, thisLine.arrow)
        ...
        // CommonUtils.log('addLine:', thisLine)
        if (__isNew) {
    ...
    loadGraphJsonData(jsonData) {
      ...
      if (window.SeeksGraphDebug) CommonUtils.log('节点预处理完毕');
    ...
    setJsonData(jsonData, callback) {
      ...
      CommonUtils.log('set jsonData::', jsonData);
      this.resetViewSize();
      if (this.graphSetting.layouts && this.graphSetting.layouts.length > 0) {
        var _defaultLayoutSetting = this.graphSetting.layouts[0];
        if (window.SeeksGraphDebug) CommonUtils.log('创建默认布局器：', this.graphSetting.layoutName);
    ...
    appendJsonData(jsonData, isRelayout, callback) {
      ...
      CommonUtils.log('appendData:', jsonData);
      this.loadGraphJsonData(jsonData);
      this.applyNewDataToCanvas();
      // this.resetViewSize()
    ...
    doLayout() {
      if (this.graphSetting.layouter && this.graphData.rootNode) {
        CommonUtils.log('需要布局的节点数量：', this.graphData.nodes.length);
    ...
    // graphOnClick(evt) {
    //   CommonUtils.log('click graph')
    // },
    wow() {
      if (window.SeeksGraphDebug) CommonUtils.log('wow.....');
    ...
    removeNodeById(nodeId) {
      ...
      for (let i = 0; i < this.lineViewList.length; i++) {
        ...
        if (thisLine.fromNode.id === nodeId || thisLine.toNode.id === nodeId) {
          // CommonUtils.log(this.lineViewList[i])
      ...
      CommonUtils.log('删除对应的线个数：', nodeId, __removed_lines);
      ...
      CommonUtils.log('删除对应的节点个数：', nodeId, __removed_nodes);
    },
    ...
    downloadAsImage(format) {
      ...
      // CommonUtils.log('window.navigator.msSaveOrOpenBlob:', window.navigator.msSaveOrOpenBlob)
      // CommonUtils.log('window.navigator.msSaveBlob：', window.navigator.msSaveBlob)
      ...
      CommonUtils.log('offset:', { _origin_offset_x, _origin_offset_y, _min_x, _min_y, _max_x, _max_y });
      ...
      CommonUtils.log('export image:', { relationGraphPosition, orign_width, orign_height, _image_width, _image_height, _min_x, _min_y, _max_x, _max_y, devicePixelRatio: window.devicePixelRatio });
      this.$nextTick(() => {
        ...
        CommonUtils.log('html2canvas:', opts);
        external___root___html2canvas___commonjs___html2canvas___commonjs2___html2canvas___amd___html2canvas___default()(exportDom, opts).then(canvas => {
          ...
          // CommonUtils.log('canvas:', fileName, dom)
          ...
          try {
            if (window.navigator.msSaveOrOpenBlob) {
              ...
              // CommonUtils.log('click to download:', opts)
              a.click();
              CommonUtils.log('click ok!');
              URL.revokeObjectURL(blob);
              CommonUtils.log('revokeObjectURL ok!');
              document.body.removeChild(a);
              CommonUtils.log('removeChild ok!');
            }
            ...
            CommonUtils.log('[SEEKS Graph]Create and download image error:', e);
          }
        });
      });
    },
    querySearchAsync(queryString, callback) {
      CommonUtils.log('fetch-suggestions', queryString);
      ...
      var rst = [];
      this.nodeViewList.forEach(thisNode => {
        CommonUtils.log('fetch:', thisNode.text);
        if (thisNode.text.indexOf(queryString) !== -1) {
          rst.push(thisNode);
        }
      });
      CommonUtils.log('fetched:', rst.length);
      callback(rst);
    },
    focusRootNode() {
      if (window.SeeksGraphDebug) CommonUtils.log('relation-graph:focusRootNode');
    ...
    handleSelect(thisNode) {
      CommonUtils.log('checked:', thisNode);
      scrollTo({
    ...
    animateStepAction(stepIndex, delay, allStepNum, stepCallback, finalCallback) {
      // CommonUtils.log(Date.now() + '步骤[' + stepIndex + ']')
    ...
    printGraphJsonData() {
      CommonUtils.log('graph options:', JSON.stringify(this.getGraphJsonOptions()));
      CommonUtils.log('graph json data:', JSON.stringify(this.getGraphJsonData()));
    }
  }
});
