...
(function (global, factory) {
  ...
  if (window.CommonUtils_2_0_0) {
    return window.CommonUtils_2_0_0;
  }
  //定义CommonUtils
  var CommonUtils = {
    //是否启用日志功能
    ...
    //后端自定义事件名称
    BACK_END_CUSTOM_EVENT_NAME_STRING: "BACK_END_CUSTOM_EVENT_NAME_STRING",
    //函数的父对象Map中的KEY
    ...
    getJoinString(joinString, ...itemStringArray) {
      var resultString = "";
      if (!itemStringArray || !itemStringArray.length || itemStringArray.length === 0) {
        return resultString;
      }
      if (itemStringArray.length === 1) {
        return itemStringArray[0];
      }
      if (!joinString) {
        joinString = "";
      }
      var firstBoolean = true;
      for (var itemString of itemStringArray) {
        if (firstBoolean) {
          firstBoolean = false;
          resultString = resultString + itemString;
        } else {
          resultString = resultString + joinString + itemString;
        }
      }
      return resultString;
    }
  ...
  window.CommonUtils_2_0_0 = CommonUtils;
  return window.CommonUtils_2_0_0;
});