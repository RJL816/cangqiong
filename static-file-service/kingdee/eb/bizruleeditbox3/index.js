(function (KDApi, $) {
  ...
  //按顺序加载CSS
  function loadFile(model, namespaceString, cssPathStringArray, currentIndexInt) {
    ...
    if ($('link[href="' + cssUriString + '"]').length === 0) {
      ...
      KDApi.loadFile(cssPathString, model, function () {
        loadFile(model, namespaceString, cssPathStringArray, currentIndexInt + 1);
      })
    } else {
      loadFile(model, namespaceString, cssPathStringArray, currentIndexInt + 1);
    }
  ...
  //载入css文件,通用型最强的放最后
  function loadCssFile(model, namespaceString, ...cssPathStringArray) {
    loadFile(model, namespaceString, cssPathStringArray, 0);
  }
  ...
  var setHtml = function (model, props) {
    ...
    //加载element_ui.css样式
    var elementUiCssPathString = 'lib/prod/element_ui/2.15.5/index.css';
    //加载vue_template.css样式
    var vueTemplateCssPathString = 'css/vue_template.css';
    //加载通用的CSS样式(作为对tailwindcss的补充)
    var commonCssPathString = 'lib/dev/common/1.0.0/common.css';
    //加载tailwind的css样式
    //var tailwindCssPathString = 'lib/dev/tailwindcss/2.2.8/tailwind.css';
    //只加载使用到的tailwind的css样式,解决CSS样式与平台冲突,减少依赖文件大小,提升响应速度
    var tailwindCssPathString = 'css/used_tailwind.css';
    loadCssFile(model, namespaceString, elementUiCssPathString, vueTemplateCssPathString, commonCssPathString, tailwindCssPathString);
      ...
      KDApi.loadFile(veuJsPathString, model, function () {
        ...
        KDApi.loadFile(elementUiJsPathString, model, function () {
          ...
          $.get(vueTemplateUrlString, {}, function (vueTemplateString) {
            ...
            model.elString = CommonUtils.getUuidString();
            //注意:由于允许同时存在多个页签,为了避免相互影响,这里使用唯一的ID
            model.dom.innerHTML = '<div id="' + model.elString + '">' + vueTemplateString + '</div>';
  ...
  //初始化VUE
  var getVm = function (model, props) {
    //字符串静态常量
    var StringConstant = {
      //类型
      "type": "type",
    ...
    var vm = new Vue({
      ...
      methods: {
        //前端方法代理
        //frontMethodNameString:前端方法名称
        //argumentArray:可变长参数
        frontMethodProxy(frontMethodNameString, ...argumentArray) {
          CommonUtils.log(frontMethodNameString, argumentArray);
          //js中有个变量arguments,可以访问所有传入的值
          if (argumentArray === null || argumentArray.length === 0) {
            //如果有前端方法名称操作则调用前端方法
            if (frontMethodNameString) {
              //调用前端方法
              vm.$options.methods[frontMethodNameString]();
            }
            return;
          }
          if (argumentArray.length === 1) {
            ...
            if (frontMethodNameString) {
              //调用前端方法
              vm.$options.methods[frontMethodNameString](argumentArray[0]);
            }
            return;
          }
          var argMap = new Map();
          ...
          if (frontMethodNameString) {
            ...
            vm.$options.methods[frontMethodNameString](argMap);
          }
        },
        //多语言
        ...
        switchRadio(argMap) {
          ...
          //这里可以直接设置
          var ruleManageRowPojo = argMap.get(StringConstant.ruleManageRowPojo);
          ruleManageRowPojo.enableBoolean = ruleManageRowPojo.enableBoolean !== true;
        ...
        getSelectionPojo(containerNode) {
          ...
          try {
            ...
            if (spanElement.parentNode) {
              var signNode = null;
              if ("SPAN" === spanElement.parentNode.nodeName) {
  ...
  //注册自定义控件
  window.KDApi.register('bizruleeditbox3', MyComponent, {
    isMulLang: true
  })
})(window.KDApi, jQuery)